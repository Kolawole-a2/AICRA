...
- **Logic:** 
  - For deterministic mapping: All controls are ransomware-relevant → precision = 1.0
...
1. **`aicra/experiments/h3_evaluation.py`**
   - Updated `compute_dac()` function (single metric)
...
1. **JSON Output:**
   - `dac_%` field (not `dac_internal_%` or `dac_external_%`)
   - Deterministic `dac_%` = 100.0%
   - Learned `dac_%` < 100.0%

2. **Actionable Precision:**
   - Deterministic precision >= Learned precision
...
3. **Markdown Summary:**
   - Single "DAC" column (not "DAC_int" and "DAC_ext")
...
3. **Verify results:**
   - Check `results/H3_full_evaluation/H3_full_results.json` has `dac_%` field
