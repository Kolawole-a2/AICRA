...
1. **`aicra/mappings/heuristic_mapping.py`**
   - Updated default config: `top_k=10`, `min_similarity=0.25` (broader, noisier)
...
2. **`aicra/experiments/h3_evaluation.py`**
   - Updated to prefer `deterministic_attack_defense_lookup.csv` (ransomware-focused)
...
**For Bash/Linux/Mac (use backslashes for line continuation):**
...
**Expected Output:**
- `data/mappings/learned_mapping.csv` with broader, generic mappings
...
**Validation:**
After generation, the sanity check will:
...
**Or use the complete reprocessing script:**
...
**If `learned_is_broader: false`, you'll see a warning in the logs and summary.**

### H3 Metrics (Expected Behavior)
...
2. **Actionable Precision & F1:**
   - Deterministic: Higher precision/F1 (fewer irrelevant controls)
   - Learned: Lower precision/F1 (includes non-ransomware controls)

3. **Risk-Score Stability:**
   - Deterministic: More stable variance/IQR behavior
...
**Solution:**
- Ensure learned mapping has MORE pairs than deterministic
...
**Solution:**
- This is a valid result if metrics are computed honestly
