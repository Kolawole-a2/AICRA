...
def main():
    ...
    parser = argparse.ArgumentParser(description="H3: Deterministic vs Learned Mapping Comparison")
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    try:
        ...
        logger.warning(f"EMBER-2024 directory not found (H3 uses pre-computed risk scores): {e}")
    ...
    try:
        ...
        if 'aggregated_metrics' in results:
            ...
            if 'improvements' in agg:
                ...
                logger.info(f"Coverage improvement: {imp.get('coverage_improvement_pct', 'N/A'):.2f}%")
                logger.info(f"Variance reduction: {imp.get('variance_reduction_pct', 'N/A'):.2f}%")
                logger.info(f"Alert fatigue reduction: {imp.get('estimated_fatigue_reduction_pct', 'N/A'):.2f}%")
