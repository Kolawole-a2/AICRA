...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="H2: Calibration and Cost-Aware Thresholding"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    try:
        ...
        if "calibration" in metrics:
            ...
            logger.info(
                f"Brier (uncalibrated): {cal.get('brier_uncalibrated', 'N/A'):.4f}"
            )
        ...
        if "improvement" in metrics:
            logger.info(
                f"Brier improvement: {metrics['improvement'].get('brier_improvement_pct', 'N/A'):.2f}%"
            )
            logger.info(
                f"ECE improvement: {metrics['improvement'].get('ece_improvement_pct', 'N/A'):.2f}%"
            )
