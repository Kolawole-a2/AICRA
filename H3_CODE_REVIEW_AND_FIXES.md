...
2. **`README.md`**
   - Updated H3 section with correct usage
   - Added note about automatic validation

3. **`docs/heuristic_mapping.md`**
   - Updated references to new canonical module

## Testing Recommendations

Before running the full evaluation, verify:

1. **Mappings are different:**
   ```bash
   python -c "
   import pandas as pd
...
2. **If mappings are identical, regenerate learned mapping:**
   ```bash
...
2. **If you get "mappings are identical" error:**
   - Regenerate learned mapping: `python -m aicra.mappings.embedding_learned_mapping`
   - Verify the new mapping is different
   - Re-run evaluation

3. **Review outputs:**
   - Check `results/H3_full_evaluation/H3_full_summary.md` for human-readable report
