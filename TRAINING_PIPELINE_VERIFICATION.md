...
- **Implementation:**
  - Small architecture: `input_dim → 128 → 64 → 2`
...
- **Features:**
  - `entropy_overall` - Overall file entropy
...
1. **Training:**
   - Extract PE static features (byte histograms, PE headers, section entropy)
   - Combine with EMBER features
   - Train LightGBM (hist, default reg, GOSS off) or small FFNN
   - Output: Model that predicts `p(ransomware)`

2. **Calibration:**
   - Load model and generate raw `p(ransomware)` predictions
   - Apply Platt or Isotonic calibration
   - Output: Calibrated **Susceptibility Score S∈[0,1]**

3. **Register Generation:**
   - Use calibrated susceptibility scores (not raw probabilities)
...
- ✅ Calibrated output is **Susceptibility Score S∈[0,1]**
- ✅ Registers use calibrated susceptibility scores (not raw probabilities)
