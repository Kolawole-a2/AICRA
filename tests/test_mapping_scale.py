...
class TestMappingScale:
    ...
    def test_batch_mapping_performance(self, temp_settings):
        ...
        ), f"Batch mapping took too long: {elapsed_time:.2f}s"
    ...
    def test_coverage_logging(self, temp_settings):
        ...
        # Log coverage report
        mapping_pipeline.log_coverage_report("test")
    ...
    def test_coverage_thresholds(self, temp_settings):
        ...
        # EMBER datasets: low alias coverage warns but does not fail
        ...
        # Non-EMBER / strict mode: low coverage should fail
        strict_settings = temp_settings.model_copy(deep=True)
        strict_settings.dataset_type = "bank_logs"
        strict_settings.require_family_mapping = True
        strict_pipeline = MappingPipeline(strict_settings, skip_mlflow=True)
        strict_pipeline.map_families_batch(unknown_families, "test")
        assert not strict_pipeline.check_coverage_thresholds("test")

        # Known families pass on EMBER (coverage gate is lenient for EMBER datasets)
        ember_pipeline = MappingPipeline(temp_settings, skip_mlflow=True)
        known_families = ["lockbit", "conti", "ryuk"] * 1000
        ember_pipeline.map_families_batch(known_families, "test")
        assert ember_pipeline.check_coverage_thresholds("test")
    ...
    def test_pattern_matching_performance(self, temp_settings):
        ...
        assert (
            elapsed_time < 2.0
        ), f"Pattern matching took too long: {elapsed_time:.2f}s"
    ...
    def test_concurrent_access(self, temp_settings):
        ...
        def worker(worker_id: int):
            ...
            result_df = mapping_pipeline.map_families_batch(
                test_families, f"worker_{worker_id}"
            )
