...
def _get_h2_split_metrics(
    results: dict[str, Any], split: str = "full_ember"
) -> dict[str, Any]:
    """Return per-split threshold metrics (full precision/recall/f1 structure)."""
    per_split = results.get("metrics", {}).get("per_split_results", [])
    for entry in per_split:
        if entry.get("split") == split:
            return entry
    if per_split:
        return per_split[0]
    pytest.skip("No per_split_results found in H2 results")
...
def test_h2_brier_score_ranges():
    ...
    assert (
        0.0 <= brier_uncal <= 1.0
    ), f"Brier (uncalibrated) must be between 0 and 1, got {brier_uncal}"
    assert (
        0.0 <= brier_cal <= 1.0
    ), f"Brier (calibrated) must be between 0 and 1, got {brier_cal}"
...
def test_h2_ece_ranges():
    ...
    assert (
        0.0 <= ece_uncal <= 1.0
    ), f"ECE (uncalibrated) must be between 0 and 1, got {ece_uncal}"
    assert (
        0.0 <= ece_cal <= 1.0
    ), f"ECE (calibrated) must be between 0 and 1, got {ece_cal}"
...
def test_h2_calibration_method():
    ...
    assert isinstance(
        method, str
    ), f"Calibration method must be a string, got {type(method)}"
    assert method in [
        "platt",
        "isotonic",
        "auto",
...
def test_h2_f1_optimized_structure():
    ...
    results_path = Path("results/H2_calibration_thresholds/H2_full_results.json")
    results = load_h2_results(results_path)
    split_metrics = _get_h2_split_metrics(results)

    f1_opt = split_metrics["f1_optimized"]
    ...
    # Aggregated summary may only store threshold + expected_loss
    agg_f1 = results["metrics"]["f1_optimized"]
    for variant in ["uncalibrated", "calibrated"]:
        assert "threshold" in agg_f1[variant]
        assert "expected_loss" in agg_f1[variant]
...
def test_h2_cost_optimized_structure():
    ...
    cost_opt = split_metrics["cost_optimized"]
    ...
    for variant in ["uncalibrated", "calibrated"]:
        ...
        for key in required_keys:
            assert (
                key in cost_metrics
            ), f"Missing key in cost_optimized.{variant}: {key}"

    agg_cost = results["metrics"]["cost_optimized"]
    assert "cost_fn" in agg_cost
    assert "cost_fp" in agg_cost
    for variant in ["uncalibrated", "calibrated"]:
        assert "threshold" in agg_cost[variant]
        assert "expected_loss" in agg_cost[variant]
...
def test_h2_threshold_ranges():
    ...
    assert (
        0.0 <= f1_uncal_thresh <= 1.0
    ), f"F1 threshold (uncalibrated) must be between 0 and 1, got {f1_uncal_thresh}"
    assert (
        0.0 <= f1_cal_thresh <= 1.0
    ), f"F1 threshold (calibrated) must be between 0 and 1, got {f1_cal_thresh}"
    ...
    assert (
        0.0 <= cost_uncal_thresh <= 1.0
    ), f"Cost threshold (uncalibrated) must be between 0 and 1, got {cost_uncal_thresh}"
    assert (
        0.0 <= cost_cal_thresh <= 1.0
    ), f"Cost threshold (calibrated) must be between 0 and 1, got {cost_cal_thresh}"
...
def test_h2_precision_recall_f1_ranges():
    ...
    for variant in ["uncalibrated", "calibrated"]:
        f1_metrics = split_metrics["f1_optimized"][variant]
        assert (
            0.0 <= f1_metrics["precision"] <= 1.0
        ), f"F1 precision ({variant}) must be between 0 and 1"
        assert (
            0.0 <= f1_metrics["recall"] <= 1.0
        ), f"F1 recall ({variant}) must be between 0 and 1"
        assert (
            0.0 <= f1_metrics["f1"] <= 1.0
        ), f"F1 score ({variant}) must be between 0 and 1"

        cost_metrics = split_metrics["cost_optimized"][variant]
        assert (
            0.0 <= cost_metrics["precision"] <= 1.0
        ), f"Cost precision ({variant}) must be between 0 and 1"
        assert (
            0.0 <= cost_metrics["recall"] <= 1.0
        ), f"Cost recall ({variant}) must be between 0 and 1"
        assert (
            0.0 <= cost_metrics["f1"] <= 1.0
        ), f"Cost F1 ({variant}) must be between 0 and 1"
...
def test_h2_expected_loss_positive():
    ...
    for variant in ["uncalibrated", "calibrated"]:
        ...
        assert (
            f1_loss >= 0.0
        ), f"F1 expected loss ({variant}) must be >= 0, got {f1_loss}"

        # Cost-optimized
        cost_loss = metrics["cost_optimized"][variant]["expected_loss"]
        assert (
            cost_loss >= 0.0
        ), f"Cost expected loss ({variant}) must be >= 0, got {cost_loss}"
...
def test_h2_cost_structure():
    ...
    assert isinstance(
        cost_fn, int | float
    ), f"cost_fn must be numeric, got {type(cost_fn)}"
    assert isinstance(
        cost_fp, int | float
    ), f"cost_fp must be numeric, got {type(cost_fp)}"
...
def test_h2_sample_count():
    ...
    assert isinstance(
        n_test, int | float
    ), f"n_test_samples must be numeric, got {type(n_test)}"
