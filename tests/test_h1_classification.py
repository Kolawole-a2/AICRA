...
def test_h1_operational_threshold_range():
    ...
    assert (
        0.0 <= threshold <= 1.0
    ), f"Operational threshold must be between 0 and 1, got {threshold}"
...
def test_h1_sample_counts():
    ...
    assert isinstance(
        n_train, int | float
    ), f"n_train_samples must be numeric, got {type(n_train)}"
    assert isinstance(
        n_test, int | float
...
def test_h1_model_type():
    ...
    assert model_type in [
        "lgbm",
        "ffnn",
    ], f"model_type must be 'lgbm' or 'ffnn', got {model_type}"
