...
def skip_broader_than_deterministic_check(monkeypatch):
    """Unit tests use tiny CSV fixtures; skip repo-wide deterministic breadth check."""
    monkeypatch.setattr(
        "aicra.mappings.heuristic_mapping.validate_learned_is_broader",
        lambda _df: None,
    )
...
def test_build_heuristic_mapping_basic(
    ...
    config = HeuristicMappingConfig(
        top_k=2,
        min_similarity=0.0,  # Allow TF-IDF fallback in CI without sentence-transformers
    ...
    assert all(
        col in result_df.columns for col in required_cols
    ), f"Missing required columns. Got: {list(result_df.columns)}"

    # Check similarity scores are in [0, 1]
    assert (
        result_df["similarity_score"].min() >= 0.0
    ), "Similarity scores should be >= 0"
    assert (
        result_df["similarity_score"].max() <= 1.0
    ), "Similarity scores should be <= 1"
    ...
    assert len(unique_techniques) == len(
        sample_attack_data
...
def test_build_heuristic_mapping_min_similarity_filtering(
    ...
    if len(result_high) > 0:
        assert (
            result_high["similarity_score"].min() >= config_high.min_similarity
        ), "All similarities should meet the high threshold"


def test_build_heuristic_mapping_top_k(
    sample_attack_data, sample_d3fend_data, temp_dir
):
    ...
    if len(result) > 0:
        ...
        assert (
            technique_counts.max() <= config.top_k
...
@pytest.mark.slow
def test_build_heuristic_mapping_deterministic(
    sample_attack_data, sample_d3fend_data, temp_dir
):
