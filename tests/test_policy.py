...
from aicra.pipelines.policy import Policy, PolicyPipeline
...
class TestPolicyPipeline:
    ...
    def test_compute_lift_at_k_report(self):
        ...
        df = pd.DataFrame({"probability": [0.1, 0.9, 0.2, 0.8], "label": [0, 1, 0, 1]})
    ...
    def test_enrich_register_with_controls(self):
        ...
        df = pd.DataFrame({"susceptibility_bucket": ["High", "Medium", "Low", "High"]})
    ...
    def test_enrich_register_with_controls_nan(self):
        ...
        df = pd.DataFrame({"susceptibility_bucket": ["High", None, "Low"]})
    ...
    def test_load_risk_bucket_controls(self, mock_mlflow, mock_file, mock_yaml):
        ...
        mock_data = {
            "__version__": "1.0.0",
            "risk_buckets": {"High": {"controls": ["Control1", "Control2"]}},
        }
