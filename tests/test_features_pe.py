...
class TestBuildPEFeatures:
    ...
    def test_build_pe_features_success(self):
        ...
        df = pd.DataFrame({"file_path": ["test_file1.bin", "test_file2.bin"]})
        ...
        assert (
            len(features_df.columns) > 256
        )  # At least byte histogram + PE + entropy features

    def test_build_pe_features_file_not_found(self):
        """Test handling of file not found errors."""
        df = pd.DataFrame({"file_path": ["nonexistent_file.bin"]})
