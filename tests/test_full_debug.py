...
class TestFullDebug:
    ...
    def temp_settings(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            families_data = {
                ...
                "mappings": {"test_family": "TestFamily", "unknown": "Unknown"},
            }
            ...
            attack_data = {
                "__version__": "1.0.0-test",
                "mappings": {"TestFamily": ["T1486", "T1055"], "Unknown": []},
            }
            ...
            d3fend_data = {
                "__version__": "1.0.0-test",
                "mappings": {"T1486": ["D3-CM001", "D3-CM002"], "T1055": ["D3-CM003"]},
            }
    ...
    def test_single_class_detection(self, temp_settings):
        ...
        features_df = pd.DataFrame(
            {"feature_0": [1, 2, 3, 4, 5], "feature_1": [0, 1, 0, 1, 0]}
        )
        ...
        with pytest.raises(RuntimeError, match="Only one class found in labels"):
            debug_pipeline.validate_data_loading(
                features_df, labels_series, families_series
            )
    ...
    def test_insufficient_data_detection(self, temp_settings):
        ...
        features_df = pd.DataFrame({"feature_0": [1, 2, 3, 4, 5]})  # Only 5 rows
        labels_series = pd.Series([0, 1, 0, 1, 0])
        families_series = pd.Series(["test", "test", "test", "test", "test"])

        # Should raise RuntimeError for insufficient data (debug mode: min 100 rows)
        with pytest.raises(
            RuntimeError, match="Insufficient data.*Need at least 100 rows"
        ):
            debug_pipeline.validate_data_loading(
                features_df, labels_series, families_series
            )
    ...
    @staticmethod
    def _make_min_debug_rows(n: int = 100) -> tuple[pd.DataFrame, pd.Series, pd.Series]:
        """Build minimal valid dataset for debug-mode validation (>= 100 rows)."""
        features_df = pd.DataFrame({"feature_0": np.arange(n, dtype=float)})
        labels_series = pd.Series([0, 1] * (n // 2))
        families_series = pd.Series(["test"] * n)
    ...
    def test_constant_feature_removal(self, temp_settings):
        ...
        features_df = pd.DataFrame(
            {
                "feature_0": np.arange(n, dtype=float),
                "feature_1": [1.0] * n,
                "feature_2": [1.0] * (n - 1) + [1.0000001],
                "feature_3": [0.0, 1.0] * (n // 2),
            }
        )
        labels_series = pd.Series([0, 1] * (n // 2))
        families_series = pd.Series(["test"] * n)
        ...
        assert (
            "feature_1"
            in debug_pipeline.debug_data["data_summary"]["constant_features"]
        )
        assert (
            "feature_2"
            in debug_pipeline.debug_data["data_summary"]["near_constant_features"]
        )
    ...
    def test_time_split_validation(self, temp_settings):
        ...
        assert (
            debug_pipeline.debug_data["split_summary"]["split_type"] == "time_ordered"
        )
    ...
    def test_debug_report_generation(self, temp_settings):
        ...
        # Create test data
        features_df = pd.DataFrame(
            {"feature_0": np.random.randn(1000), "feature_1": np.random.randn(1000)}
        )
        ...
        train_data = pd.DataFrame(
            {"feature_0": np.random.randn(800), "feature_1": np.random.randn(800)}
        )
        test_data = pd.DataFrame(
            {"feature_0": np.random.randn(200), "feature_1": np.random.randn(200)}
        )
        ...
        report_path = debug_pipeline.generate_debug_report(
            test_metrics, None, training_summary
        )
        ...
        assert (
            len(report_data["probable_causes"]) > 0
        )  # Should have causes for low AUROC
    ...
    def test_lightgbm_retuning(self, temp_settings):
        ...
        assert (
            training_summary["n_features_used"] <= n_features
        )  # May be less due to cleaning
    ...
    def test_debug_artifacts_creation(self, temp_settings):
        ...
        # Create test data with a constant feature so removal artifact is written
        n = 100
        features_df = pd.DataFrame(
            {
        ...
        labels_series = pd.Series(np.random.randint(0, 2, n))
        families_series = pd.Series(["test"] * n)
    ...
    def test_warning_accumulation(self, temp_settings):
        ...
        features_df = pd.DataFrame(
            {
                "feature_0": np.arange(n, dtype=float),
                "feature_1": [np.nan, np.nan, np.nan, 1.0, 0.0] * (n // 5),
            }
        )
        labels_series = pd.Series([0, 1] * (n // 2))
        families_series = pd.Series(["test"] * n)
