# Precision-Recall Trade-off in Banking Security: Why ~64.8% Precision is Acceptable

## Executive Summary

AICRA achieves **64.8% precision** and **99.8% recall** using a banking-optimized threshold (0.0248). While precision is lower than the baseline (77.34%), this trade-off is **intentional and appropriate** for banking security, where missing ransomware (false negatives) is far more costly than investigating false positives.
...
- **Precision** = TP / (TP + FP) = "Of all alerts, what percentage are real threats?"
  - AICRA: 64.8% = 4,585 true positives / (4,585 + 2,493 false positives)
  - **Interpretation**: Roughly 2 out of every 3 alerts are actual ransomware

- **Recall** = TP / (TP + FN) = "Of all real threats, what percentage did we catch?"
  - AICRA: 99.8% = 4,585 true positives / (4,585 + 7 false negatives)
  - **Interpretation**: Only 7 ransomware samples missed out of 4,592 total
...
AICRA uses a **very low threshold (0.0248)** to maximize recall, accepting lower precision.
...
The banking-optimized threshold (0.0248) minimizes **Expected Loss**:
...
|--------|-------|----------------|
| **Precision** | 0.6478 (64.8%) | 2 out of 3 alerts are real ransomware |
| **Recall** | 0.9985 (99.8%) | Only 7 ransomware missed out of 4,592 |
| **AUROC** | 0.9796 | Excellent discrimination ability |
| **PR-AUC** | 0.9767 | Strong precision-recall balance |
...
```
                Predicted
              Benign  Ransomware
Actual Benign   2916      2493    (FP = 2,493)
Ransomware        7       4585    (TP = 4,585)
```

**Key Numbers:- **True Positives**: 4,585 ransomware correctly identified
- **False Positives**: 2,493 benign files flagged (will be investigated)
- **False Negatives**: 7 ransomware missed (critical metric, very low)
- **True Negatives**: 2,916 benign files correctly ignored

### Comparison with Baseline

| Metric | Baseline | AICRA | Change | Interpretation |
|--------|----------|-------|--------|----------------|
| **Precision** | 0.7734 | 0.6478 | **-16.2%** | Lower precision (intentional) |
| **Recall** | 0.6363 | 0.9985 | **+56.9%** | Massive recall improvement |
| **F1** | 0.6982 | 0.7858 | **+12.5%** | Overall better balance |
| **FN Rate** | 36.4% | 0.15% | **-99.6%** | Critical improvement |

**Key Insight**: The baseline has higher precision (77.34%) but **misses 36% of ransomware**, which is unacceptable for banking security.

## Why 64.8% Precision is Acceptable

### 1. Operational Efficiency

**64.8% precision means:- **2 out of 3 alerts are real threats** - This is a manageable ratio for SOC analysts
...
**99.8% recall means:- **Only 7 ransomware samples missed** out of 4,592 total
...
**Cost Analysis:- **Missed ransomware cost**: 7 FNs × $5M = $35M potential impact
- **False positive cost**: 2,493 FPs × $100 = $249,300 investigation cost
- **Total cost**: $35.25M (dominated by FN risk)
...
- **64.8% precision** is acceptable when combined with high recall
...
### Why 64.8% Precision is "Suitable"

1. **Meets AUROC requirement**: 0.9796 >> 0.95 ✓
2. **Operational precision suitable**: 64.8% is suitable because:
   - Combined with 99.8% recall, it provides comprehensive threat coverage
   - False positive rate (35.2%) is manageable with proper SOC workflows
...
2. **Tier 1 Analysis**: Quick triage of alerts:
   - 64.8% are real threats → Escalate to Tier 2
   - 35.2% are false positives → Dismiss after quick review
...
**Efficiency**: With 64.8% precision, SOC analysts can efficiently triage alerts, and the high recall ensures no critical threats are missed.
...
| **AICRA (Banking)** | **64.8%** | **99.8%** | Banking-optimized |
...
- AICRA at banking threshold: 64.8% precision, 99.8% recall (FN-prioritized trade-off)
...
|-----------|-----------|--------|----------|---------------|
| **0.0248 (Current)** | 64.8% | 99.8% | 7 | Optimal for banking |
...
**Conclusion**: The current threshold (0.0248) is optimal for banking security, balancing precision and recall appropriately.

## Defense Talking Points

### When Asked: "Is 64.8% precision too low?"
...
3. **Highlight recall**: "We achieve 99.8% recall, meaning we catch almost all ransomware. Only 7 samples are missed out of 4,592."

4. **Show operational suitability**: "64.8% precision means 2 out of 3 alerts are real threats, which is manageable for SOC analysts with proper triage workflows."
...
3. **Operational impact**: "The current threshold (0.0248) minimizes expected loss under banking cost structure, ensuring optimal operational performance."

## Conclusion

**64.8% precision is acceptable and appropriate for banking security** because:
...
The lower precision is a **strategic choice** optimized for banking security, not a model limitation. The combination of 64.8% precision and 99.8% recall provides **operational precision suitable for banking environments** as stated in the H1 hypothesis.
