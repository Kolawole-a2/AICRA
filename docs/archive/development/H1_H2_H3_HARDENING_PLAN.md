...
**Files to create:- `experiments/h1_train_eval.py` - Wrapper around h1_classification.py
...
**Requirements:- Use `get_ember2024_dir()` from `aicra.utils.data_paths`
- Write outputs to `artifacts/`
- Log seeds, configs, timestamps
- Runnable from repo root

### B) Time-Ordered Split + Out-of-Family Test
**Changes needed:- Verify time-ordered split is used consistently in H1
...
**H1:- ✅ AUROC, Precision, Recall, F1 (already computed)
...
**H2:- ✅ Brier, ECE pre/post (already computed)
- Add: Temporal calibration check (calibrate on earlier window, test on later)

**H3:- ✅ Coverage, consistency, precision, variance (already computed)
...
**New requirement:- Add hard check in H3: if learned mapping == deterministic mapping, fail with error
