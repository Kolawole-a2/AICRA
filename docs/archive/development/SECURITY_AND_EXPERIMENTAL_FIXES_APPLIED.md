...
**Files Fixed:1. ✅ `aicra/utils/policy_writer.py` - Added `safe_load_npz()` with path validation
...
**Changes:- Replaced all `np.load(..., allow_pickle=True)` with `allow_pickle=False`
...
**Changes:- Changed port bindings from `"8000:8000"` to `"127.0.0.1:8000:8000"` (localhost only)
...
**Changes:- Changed `flake8 src/ notebooks/` → `flake8 aicra/ tests/`
...
**Changes:- Added `time_ordered` parameter to `load_ember_2024()` function
...
**Features:- `evaluate_temporal_holdout()` - Evaluates on strictly future time periods
...
**Features:- `evaluate_temporal_calibration_drift()` - Evaluates calibration drift between time windows T1 and T2
...
**Content:- Explains banking-specific cost asymmetry (C_FN = $5M, C_FP = $1)
...
**Features:- `perturb_features()` - Adds Gaussian, uniform, or mimicry perturbations
...
**Content:- Documents evaluation framework
...
✅ **All changes are backward compatible:- `load_ember_2024()` defaults to `time_ordered=False` (existing behavior preserved)
