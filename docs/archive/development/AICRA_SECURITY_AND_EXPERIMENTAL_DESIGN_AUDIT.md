...
#### **Issue SummaryFound **4 locations** using `allow_pickle=True`, which can execute arbitrary code if loading untrusted files.

#### **Locations Found| File | Line | Context | Input Source | Risk Level |
|------|------|---------|--------------|------------|
...
#### **Proposed Fixes**File: `aicra/utils/policy_writer.py````python
# BEFORE (UNSAFE)
...
--**File: `aicra/utils/train_ffnn.py````python
# BEFORE (UNSAFE)
...
--**File: `aicra/utils/evaluate.py````python
# BEFORE (UNSAFE)
ns = np.load(args.predictions, allow_pickle=True)
...
--**File: `aicra/utils/train_lightgbm.py````python
# BEFORE (UNSAFE)
f = np.load(args.features, allow_pickle=True)
# ... (line 39)
...
#### **Summary- **All 7 unsafe `np.load` calls** should be replaced with `allow_pickle=False` + path validation.
...
#### **Issue SummaryDocker Compose exposes ports **8000** (AICRA app) and **5000** (MLflow) to `0.0.0.0` without authentication.

#### **Current Configuration**File: `docker-compose.yml````yaml
services:
  aicra:
    ports:
...
#### **Threat Model- **Port 8000 (AICRA):** If AICRA exposes an API/dashboard, unauthenticated access could allow:
...
#### **Proposed Hardening**File: `docker-compose.yml````yaml
# BEFORE
services:
  aicra:
    ports:
...
**Additional Recommendation: Create `docker-compose.prod.yml````yaml
...
#### **Issue Summary**File: `.github/workflows/lint.yml`** references non-existent `src/` directory.

#### **Current Configuration**File: `.github/workflows/lint.yml````yaml
# BEFORE (WRONG)
- name: Run Flake8
...
#### **Gap Analysis Table| File | Location | Promise | Actual Behavior | Gap |
|------|----------|---------|-----------------|-----|
...
#### **Key Findings1. **Out-of-family evaluation:** ✅ **WORKING** - H1 correctly implements OOF test.
...
#### **Proposed Fix: Time-Ordered Split in `load_ember_2024()`**File: `aicra/core/data.py````python
# BEFORE
...
#### **Proposed: Out-of-Sample Evaluation Script**New File: `aicra/experiments/h1_out_of_sample_eval.py````python
"""
...
#### **Proposed: Temporal Calibration Evaluation**New File: `aicra/pipelines/temporal_calibration.py````python
"""
...
#### **Calibration Locations| File | Method | Implementation |
|------|--------|----------------|
...
#### **Key FindingCurrent implementation is **standard cost-optimization** - minimizes `FN_cost * FN + FP_cost * FP`. The **novelty** is not in the algorithm, but in:
1. **Banking-specific cost asymmetry** (FN >> FP)
2. **Integration with Expected Loss** (probability × impact)
3. **Alignment with ATT&CK-D3FEND risk registers--### C2. Proposed Novelty Documentation

**New File: `docs/novelty_threshold_calibration.md````markdown
...
- Calibration: isotonic regression on held-out probabilities (`aicra/pipelines/calibration.py`)
...
#### **Search Results**Found:** ❌ **NO existing adversarial robustness evaluation- No mentions of "adversarial", "FGSM", "PGD", "mimicry", "evasion" in codebase
...
#### **New File: `aicra/experiments/h1_adversarial_eval.py````python
"""
...
#### **New File: `docs/adversarial_limitations.md````markdown
