...
- **Contains:  - Research design explanation
...
### Main Plots:
1. **`dac_internal_per_split.png`   - DAC_internal comparison across splits
   - Shows deterministic (100%) vs learned mapping performance

2. **`dac_per_split.png`   - DAC_external comparison across splits
   - Secondary benchmark metric

3. **`precision_per_split.png`   - Actionable precision comparison
   - Deterministic vs learned mapping precision

4. **`variance_reduction_per_split.png`   - Risk score variance reduction comparison
   - Shows stability improvements

5. **`summary_metrics.png`   - Summary visualization of key metrics
