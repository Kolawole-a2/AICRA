...
**Key Techniques:**
- T1486 (Data Encrypted for Impact) → D3-BDR, D3-BAC, D3-SAW
...
**Test Phases (defined in `aicra/pipelines/test_runner.py`):**

1. **smoke** - Minimal test with synthetic data
...
**Time-Ordered Splits (defined in `aicra/core/splits.py`):**
- `time_ordered_split()` - Splits by timestamp
- Default ratios: 70% train, 15% val, 15% test
- Ensures temporal ordering (no data leakage)

## DAC Integration Points

**Where DAC logic will integrate:**
1. **New module:** `aicra/metrics/dac.py` - Core DAC computation
