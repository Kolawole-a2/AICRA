...
def normalize_mapping(df: pd.DataFrame) -> dict[str, set[str]]:
    ...
    if tech_col is None or ctrl_col is None:
        raise ValueError(
            f"Cannot find technique and control columns in mapping. Found columns: {list(df.columns)}"
        )
...
def classify_technique_overlap(
    det_controls: set[str], learned_controls: set[str]
) -> str:
...
def load_risk_scores_techniques(splits_config_path: Path, repo_root: Path) -> set[str]:
    ...
    for split_name, risk_scores_rel_path in splits.items():
        ...
        if not risk_scores_path.exists():
            logger.warning(
                f"Risk scores not found for split '{split_name}': {risk_scores_path}"
            )
            continue
        ...
        try:
            ...
            if tech_col is None:
                logger.warning(
                    f"No technique_id column found in {risk_scores_path}. Columns: {list(df.columns)}"
                )
                continue
    ...
    logger.info(
        f"Total unique techniques across all risk scores: {len(all_techniques)}"
    )
...
def diagnose_mapping_overlap(
    ...
    logger.info(
        f"   Deterministic: {len(det_mapping)} techniques, {sum(len(v) for v in det_mapping.values())} pairs"
    )
    logger.info(
        f"   Learned: {len(learned_mapping)} techniques, {sum(len(v) for v in learned_mapping.values())} pairs"
    )
    ...
    logger.info(
        f"   Global Jaccard: {global_jaccard:.4f} ({global_jaccard * 100:.2f}%)"
    )
    ...
    if risk_scores_path and risk_scores_path.exists():
        ...
        if tech_col:
            ...
            logger.info(
                f"   Loaded {len(risk_score_techniques)} techniques from provided risk scores"
            )
        else:
            risk_score_techniques = load_risk_scores_techniques(
                splits_config_path, repo_root
            )
    else:
        risk_score_techniques = load_risk_scores_techniques(
            splits_config_path, repo_root
    ...
    if risk_score_techniques:
        logger.info(
            f"   Restricting to {len(risk_score_techniques)} techniques present in risk scores"
        )
    ...
    if risk_score_techniques:
        logger.info("  (Restricted to techniques in risk scores)")
    ...
    if global_jaccard > 0.90:
        ...
        logger.warning(
            "Consider regenerating learned mapping with increased top_k or different parameters."
        )
        logger.warning("=" * 80)
    ...
    if len(exact_matches) > len(technique_classifications) * 0.8:
        logger.warning("\n" + "=" * 80)
        logger.warning(
            f"⚠️  WARNING: {len(exact_matches)}/{len(technique_classifications)} techniques have EXACT_MATCH mappings!"
        )
        logger.warning("=" * 80)
        ...
        logger.warning(
            "Consider regenerating learned mapping with increased diversity."
        )
        logger.warning("=" * 80)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if not risk_scores_path.exists():
            logger.warning(
                f"Provided risk scores path does not exist: {risk_scores_path}"
            )
            risk_scores_path = None
    ...
    try:
        ...
        print(
            f"EXACT_MATCH techniques: {results['overlap_classification']['exact_match_count']}"
        )
        print(
            f"PARTIAL_OVERLAP techniques: {results['overlap_classification']['partial_overlap_count']}"
        )
        print(
            f"DISJOINT techniques: {results['overlap_classification']['disjoint_count']}"
        )
        ...
        if results.get("risk_score_coverage"):
            ...
            print(
                f"  EXACT_MATCH: {rsc['exact_match_count']} ({rsc['exact_match_fraction'] * 100:.1f}%)"
            )
