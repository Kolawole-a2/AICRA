...
def plot_roc_curve(y_true: np.ndarray, y_prob: np.ndarray, auroc: float, out_path: Path) -> None:
    """Plot ROC curve."""
...
def plot_pr_curve(y_true: np.ndarray, y_prob: np.ndarray, pr_auc: float, out_path: Path) -> None:
    ...
    plt.title("Precision-Recall Curve - Canonical H1 Experiment", fontsize=14, fontweight="bold")
...
def plot_reliability_diagram(
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(prob_pred, prob_true, "s-", label="Calibration Curve", linewidth=2, markersize=8)
...
def generate_h1_plots(repo_root: Path) -> None:
    ...
    with open(h1_results_path, "r") as f:
    ...
    if use_multi_split:
        ...
        if not per_split_results:
            logger.warning("Multi-split mode but no per-split results found, using full test set")
        ...
        if use_multi_split:
            ...
            for split_name, (start_idx, end_idx) in splits.items():
                ...
                plot_roc_curve(
                    split_labels, y_prob, auroc,
                    split_plots_dir / "roc.png"
                )
                plot_pr_curve(
                    split_labels, y_prob, pr_auc,
                    split_plots_dir / "pr.png"
                )
                plot_confusion_matrix(
                    split_labels, y_pred, threshold,
                    split_plots_dir / "confusion.png"
                )
...
def generate_h2_plots(repo_root: Path) -> None:
    ...
    with open(h2_results_path, "r") as f:
