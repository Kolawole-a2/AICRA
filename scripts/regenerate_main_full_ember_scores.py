...
if not data_dir.exists():
    ...
    print(f"  Or set data_dir in config/settings.yaml")
    sys.exit(1)
...
try:
    ...
    print(f"  Prediction stats: mean={risk_scores.mean():.6f}, std={risk_scores.std():.6f}, unique={np.unique(risk_scores).size}")
    ...
    h3_main["technique_id"] = h3_main["technique_id"].fillna('T1486').replace('', 'T1486')
...
try:
    ...
    h3_full["technique_id"] = h3_full["technique_id"].fillna('T1486').replace('', 'T1486')
