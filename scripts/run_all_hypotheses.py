...
def run_h1_experiment(repo_root: Path, output_dir: Path | None = None) -> Path:
...
def run_h2_experiment(repo_root: Path, output_dir: Path | None = None) -> Path:
    """Run H2 calibration and thresholding experiment."""
    logger.info("=" * 80)
    logger.info("Running H2: Calibration and Cost-Aware Thresholding")
    logger.info("=" * 80)

    from aicra.experiments.h2_calibration_thresholds import (
...
def run_h3_experiment(repo_root: Path, output_dir: Path | None = None) -> Path:
    ...
    if not learned_mapping.exists():
        ...
        try:
            ...
            raise NotImplementedError(
                "Please generate learned_mapping.csv first using generate_learned_mapping.py"
            )
