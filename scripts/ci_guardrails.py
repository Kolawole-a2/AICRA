...
def get_tracked_files() -> List[str]:
    ...
    try:
        ...
        print("ERROR: git not found. This script must be run in a git repository.", file=sys.stderr)
        sys.exit(1)
...
def main() -> int:
    ...
    for filepath_str in tracked_files:
        ...
        if violations:
            for line_num, pattern, message in violations:
                ...
                errors.append(f"{filepath_str}:{line_num}: {message} (found '{pattern}')")
    ...
    if errors:
        ...
        print("4. Fix unsafe code patterns (use allow_pickle=False or safe alternatives)")
