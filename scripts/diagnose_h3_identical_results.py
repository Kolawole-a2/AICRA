...
def diagnose_identical_results(
    ...
    det_pairs = set(zip(det_df[det_tech_col].dropna(), det_df[det_ctrl_col].dropna()))
    learned_pairs = set(zip(learned_df["technique_id"].dropna(), learned_df["control_id"].dropna()))
    ref_pairs = set(zip(ref_df["technique_id"].dropna(), ref_df["control_id"].dropna()))
    
    det_techniques = set(det_df[det_tech_col].dropna().unique())
    learned_techniques = set(learned_df["technique_id"].dropna().unique())
    
    logger.info(f"   Deterministic: {len(det_pairs)} pairs, {len(det_techniques)} techniques")
    logger.info(f"   Learned: {len(learned_pairs)} pairs, {len(learned_techniques)} techniques")
    ...
    logger.info(f"   ✓ Mappings are different")
    ...
    if missing_in_learned:
        logger.warning(f"   ⚠ Learned mapping missing {len(missing_in_learned)} techniques from deterministic:")
        logger.warning(f"      {sorted(missing_in_learned)}")
        logger.warning("   This may cause issues if risk scores contain these techniques.")
    
    if extra_in_learned:
        logger.info(f"   ✓ Learned mapping has {len(extra_in_learned)} extra techniques (OK)")
    ...
    if common_techniques:
        ...
        for tech in common_techniques:
            det_controls = set(det_df[det_df[det_tech_col] == tech][det_ctrl_col].dropna())
            learned_controls = set(learned_df[learned_df["technique_id"] == tech]["control_id"].dropna())
        ...
        logger.info(f"   Techniques with IDENTICAL controls: {len(identical_techniques)}")
        ...
        logger.info(f"   Techniques with DIFFERENT controls: {len(different_techniques)}")
    ...
    if risk_scores_path and risk_scores_path.exists():
        ...
        if in_both_mappings:
            ...
            if len(identical_in_risk) == len(in_both_mappings):
                logger.error("   ✗ PROBLEM: All techniques in risk scores have IDENTICAL mappings!")
                logger.error("   This is why H3 produces identical results.")
                logger.error("   Solution: Regenerate learned mapping with different top_k or parameters")
            elif identical_in_risk:
                logger.warning(f"   ⚠ {len(identical_in_risk)}/{len(in_both_mappings)} techniques have identical mappings")
    ...
    if det_pairs == learned_pairs:
        ...
        logger.error("SOLUTION: Run 'python scripts/fix_learned_mapping_for_h3.py'")
    elif len(identical_techniques) == len(common_techniques) if common_techniques else False:
        logger.error("ROOT CAUSE: All common techniques have identical control mappings")
        logger.error("SOLUTION: Regenerate learned mapping with top_k=4 or top_k=5 to increase diversity")
    elif risk_scores_path and risk_scores_path.exists() and len(identical_in_risk) == len(in_both_mappings):
        logger.error("ROOT CAUSE: All techniques in risk scores have identical mappings")
        logger.error("SOLUTION: Regenerate learned mapping or use risk scores with different techniques")
    else:
        logger.info("Mappings appear different. If H3 still shows identical results:")
        logger.info("1. Check that risk scores contain techniques with different mappings")
        logger.info("2. Verify H3 is loading the correct mapping files")
        logger.info("3. Check H3 evaluation code for any filtering/normalization issues")
