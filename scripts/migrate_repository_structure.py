...
def find_python_files(root: Path) -> list[Path]:
    ...
    for path in root.rglob("*.py"):
        ...
        if any(
            skip in str(path)
            for skip in [".venv", "venv", "__pycache__", ".git", "build", "dist"]
        ):
            continue
    ...
    try:
        ...
        for old_import, new_import in IMPORT_MAPPINGS.items():
            ...
            if re.search(pattern_from, content):
                ...
                changes.append(
                    f"Updated 'from {old_import} import' to 'from {new_import} import'"
                )
            ...
            if re.search(pattern_import, content):
                ...
                changes.append(
                    f"Updated 'import {old_import}' to 'import {new_import}'"
                )
    ...
    for old_path_str, new_path_str in FILE_MOVEMENTS.items():
        ...
        if old_path.exists():
            if not dry_run:
                ...
                all_changes[str(new_path)] = [
                    f"Moved from {old_path_str} to {new_path_str}"
                ]
            else:
                all_changes[str(new_path)] = [
                    f"[DRY RUN] Would move {old_path_str} to {new_path_str}"
                ]
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate repository to new structure")
    parser.add_argument(
        "--dry-run",
        action="store_true",
