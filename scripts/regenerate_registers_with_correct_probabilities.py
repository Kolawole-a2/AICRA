...
from aicra.register import Policy, compute_register, write_register
from aicra.utils.register_validation import (
    validate_register_probabilities,
    validate_susceptibility_buckets,
)
...
def main():
    parser = argparse.ArgumentParser(
        description="Regenerate risk registers with correct probabilities"
    )
    ...
    if debug:
        print(
            f"[DEBUG] Absolute working directory: {os.path.abspath(cwd)}",
            file=sys.stdout,
        )
    ...
    if debug:
        print("[DEBUG] Checking model paths:", file=sys.stdout)
    ...
    if model is None:
        ...
        print(
            "  Please run: python -m aicra.run-test --phase small_ember",
            file=sys.stderr,
        )
        print(
            "  This will train and save the model needed for regeneration.",
            file=sys.stderr,
        )
    ...
    try:
        settings = Settings()
        print("  [OK] Settings loaded", file=sys.stdout)
    except Exception as e:
    ...
    def load_policy() -> Policy:
        ...
        if debug:
            print("[DEBUG] Checking policy paths:", file=sys.stdout)
            for policy_path in policy_paths:
                abs_path = os.path.abspath(policy_path)
                exists = policy_path.exists()
                print(
                    f"[DEBUG]   {policy_path} -> {abs_path} (exists: {exists})",
                    file=sys.stdout,
                )
        ...
        for policy_path in policy_paths:
            if policy_path.exists():
                try:
                    ...
                    if debug:
                        print(
                            f"[DEBUG] Loading policy from: {abs_path}", file=sys.stdout
                        )
                    ...
                    print(
                        f"  [ERROR] Could not load policy from {policy_path}: {e}",
                        file=sys.stderr,
                    )
                    continue
    ...
    policy = load_policy()
    print(
        f"  Policy: Threshold={policy.threshold}, Impact=${policy.impact_default:,.2f}",
        file=sys.stdout,
    )
    ...
    try:
        ...
        if jsonl_files:
            ...
            print(
                f"  [OK] Found data directory: {data_dir} ({len(jsonl_files)} JSONL files)",
                file=sys.stdout,
            )
            if debug:
                print(
                    f"[DEBUG] Absolute data directory: {abs_data_dir}", file=sys.stdout
                )
                print("[DEBUG] JSONL files:", file=sys.stdout)
                for f in jsonl_files[:5]:  # Show first 5
                    print(f"[DEBUG]   {os.path.abspath(f)}", file=sys.stdout)
                if len(jsonl_files) > 5:
                    print(
                        f"[DEBUG]   ... and {len(jsonl_files) - 5} more",
                        file=sys.stdout,
                    )
        else:
            print(
                f"  [WARNING] Data directory found but no JSONL files: {data_dir}",
                file=sys.stderr,
            )
            data_dir = None
    ...
    if data_dir is None:
        ...
        if debug:
            print("[DEBUG] Trying fallback data directories:", file=sys.stdout)
            for d in possible_data_dirs:
                abs_d = os.path.abspath(d)
                exists = d.exists()
                jsonl_count = len(list(d.glob("*.jsonl"))) if exists else 0
                print(
                    f"[DEBUG]   {d} -> {abs_d} (exists: {exists}, jsonl: {jsonl_count})",
                    file=sys.stdout,
                )
        ...
        for d in possible_data_dirs:
            if d.exists():
                ...
                if jsonl_files:
                    ...
                    if debug:
                        print(
                            f"[DEBUG] Absolute data directory: {abs_data_dir}",
                            file=sys.stdout,
                        )
                    break
    ...
    if data_dir is None:
        ...
        print("  Please ensure EMBER data is available.", file=sys.stderr)
        print(
            "  You can set AICRA_EMBER2024_DIR environment variable.", file=sys.stderr
        )
        raise FileNotFoundError(
            "Data directory not found. Checked paths but none contained JSONL files."
        )
    ...
    def regenerate_register_with_model(
        ...
        try:
            ...
            if debug:
                abs_data_dir = os.path.abspath(data_dir)
                print(
                    f"[DEBUG] Loading from absolute path: {abs_data_dir}",
                    file=sys.stdout,
                )

            features_df, labels_series, families_series, metadata = (
            ...
            print(
                f"      Label value counts: {pd.Series(label_values).value_counts().to_dict()}",
                file=sys.stdout,
            )
            ...
            print(
                f"      [OK] Label validation passed: {len(label_counts)} classes",
                file=sys.stdout,
            )
            ...
            print(
                f"      Raw predictions: min={y_prob_raw.min():.6f}, max={y_prob_raw.max():.6f}, mean={y_prob_raw.mean():.6f}",
                file=sys.stdout,
            )
            if debug:
                print(
                    f"[DEBUG] Raw prediction shape: {y_prob_raw.shape}", file=sys.stdout
                )
                print(
                    f"[DEBUG] Raw prediction dtype: {y_prob_raw.dtype}", file=sys.stdout
                )
            ...
            if calibrator_path.exists():
                ...
                if debug:
                    print(
                        f"[DEBUG] Loading calibrator from: {abs_calibrator_path}",
                        file=sys.stdout,
                    )
                print(
                ...
                if debug:
                    print(
                        f"[DEBUG] No calibrator found at: {os.path.abspath(calibrator_path)}",
                        file=sys.stdout,
                    )
                print(
                    "\n  [3] No calibrator found, using raw probabilities...",
                    file=sys.stdout,
                )
            ...
            print(
                f"      Probability array shape: {probabilities.shape}", file=sys.stdout
            )
            ...
            print(
                f"      Probability range: [{prob_min:.6f}, {prob_max:.6f}]",
                file=sys.stdout,
            )
            ...
            print("      [OK] Probability range validation passed", file=sys.stdout)
            ...
            print(
                f"      Mean probability for benign (label=0): {mean_prob_neg:.6f}",
                file=sys.stdout,
            )
            print(
                f"      Mean probability for ransomware (label=1): {mean_prob_pos:.6f}",
                file=sys.stdout,
            )
            ...
            print("      [OK] Probability direction validation passed", file=sys.stdout)

            print("\n  [4] Prediction quality check:", file=sys.stdout)
            print(
                f"      Benign (label=0): mean={benign_probs.mean():.6f}, median={np.median(benign_probs):.6f}, max={benign_probs.max():.6f}",
                file=sys.stdout,
            )
            print(
                f"      Ransomware (label=1): mean={ransomware_probs.mean():.6f}, median={np.median(ransomware_probs):.6f}, min={ransomware_probs.min():.6f}",
                file=sys.stdout,
            )
            ...
            if debug:
                print(
                    f"[DEBUG] Register dataframe shape: {register_df.shape}",
                    file=sys.stdout,
                )
                print(
                    f"[DEBUG] Register dataframe columns: {list(register_df.columns)}",
                    file=sys.stdout,
                )
                print("[DEBUG] Register dataframe head(3):", file=sys.stdout)
                print(
                    register_df[["family", "probability", "label"]].head(3).to_string(),
                    file=sys.stdout,
                )
            ...
            print(
                "      Bucketing thresholds: Low [0.0, 0.33], Medium (0.33, 0.66], High (0.66, 1.0]",
                file=sys.stdout,
            )
            print(
                "      Comparison: Low uses <=0.33, Medium uses >0.33 and <=0.66, High uses >0.66",
                file=sys.stdout,
            )

            if "susceptibility_bucket" not in register_df.columns:
                raise ValueError(
                    "Missing 'susceptibility_bucket' column after compute_register"
                )

            # Cross-tabulation: bucket counts overall and by label
            bucket_counts = register_df["susceptibility_bucket"].value_counts()
            print("      Bucket counts (overall):", file=sys.stdout)
            ...
            if "label" in register_df.columns:
                cross_tab = pd.crosstab(
                    register_df["label"],
                    register_df["susceptibility_bucket"],
                    margins=True,
                )
                print(
                    "      Bucket counts by label (cross-tabulation):", file=sys.stdout
                )
                ...
                benign_high_pct = (
                    (benign_high_count / benign_total * 100)
                    if benign_total > 0
                    else 0.0
                )

                print(
                    f"      Benign samples in High bucket: {benign_high_count}/{benign_total} ({benign_high_pct:.2f}%)",
                    file=sys.stdout,
                )

                if benign_high_pct > 2.0:
                    print(
                        f"      [WARNING] {benign_high_pct:.2f}% of benign samples are in High bucket (>2% threshold)",
                        file=sys.stderr,
                    )
                    if debug:
                        # Show top 20 benign rows by probability
                        benign_high_rows = benign_df[
                            benign_df["susceptibility_bucket"] == "High"
                        ].nlargest(20, "probability")
                        print(
                            "[DEBUG] Top 20 benign rows in High bucket by probability:",
                            file=sys.stdout,
                        )
                        print(
                            benign_high_rows[
            ...
            print("      [OK] Bucketing validation completed", file=sys.stdout)
            ...
            try:
                ...
                print(
                    f"      WARNING: Could not enrich with controls: {e}",
                    file=sys.stderr,
                )
            ...
            try:
                ...
                print("      [OK] Probability validation passed", file=sys.stdout)
            except ValueError as e:
                print(
                    f"      [ERROR] Probability validation failed: {e}", file=sys.stderr
                )
                raise
            ...
            try:
                ...
                print("      [OK] Bucket validation passed", file=sys.stdout)
            except ValueError as e:
            ...
            print(
                f"      Benign - Low bucket: {(benign_final['susceptibility_bucket'] == 'Low').sum()}/{len(benign_final)}",
                file=sys.stdout,
            )
            print(
                f"      Benign - Medium bucket: {(benign_final['susceptibility_bucket'] == 'Medium').sum()}/{len(benign_final)}",
                file=sys.stdout,
            )
            print(
                f"      Benign - High bucket: {(benign_final['susceptibility_bucket'] == 'High').sum()}/{len(benign_final)}",
                file=sys.stdout,
            )
            print(
                f"      Ransomware - Low bucket: {(ransomware_final['susceptibility_bucket'] == 'Low').sum()}/{len(ransomware_final)}",
                file=sys.stdout,
            )
            print(
                f"      Ransomware - Medium bucket: {(ransomware_final['susceptibility_bucket'] == 'Medium').sum()}/{len(ransomware_final)}",
                file=sys.stdout,
            )
            print(
                f"      Ransomware - High bucket: {(ransomware_final['susceptibility_bucket'] == 'High').sum()}/{len(ransomware_final)}",
                file=sys.stdout,
            )
            ...
            print("      [OK] Register saved", file=sys.stdout)
            print(f"        Latest: {os.path.abspath(latest_path)}", file=sys.stdout)
            print(
                f"        Archived: {os.path.abspath(archived_path)}", file=sys.stdout
            )
            ...
            if register_csv.exists():
                print(
                    f"        Register CSV: {os.path.abspath(register_csv)}",
                    file=sys.stdout,
                )
            if register_json.exists():
                print(
                    f"        Register JSON: {os.path.abspath(register_json)}",
                    file=sys.stdout,
                )
    ...
    regeneration_results["small_ember"] = regenerate_register_with_model(
        "small_ember", sample_size=2000
    )

    # Full EMBER (all samples)
    regeneration_results["full_ember"] = regenerate_register_with_model(
        "full_ember", sample_size=None
    )
    ...
    if successful == total:
        print(
            f"\n[SUCCESS] All {successful}/{total} registers regenerated with correct model predictions!",
            file=sys.stdout,
        )
        print("\nThe registers now have:", file=sys.stdout)
        print(
            "  - Correct probability values from model.predict_proba()", file=sys.stdout
        )
        print("  - Calibrated probabilities (if calibrator available)", file=sys.stdout)
        print(
            "  - Proper susceptibility scores (probability.clip(0,1))", file=sys.stdout
        )
        print("  - Correct susceptibility buckets (Low/Medium/High)", file=sys.stdout)
        print(
            "  - Correct expected loss (susceptibility × $5,000,000)", file=sys.stdout
        )
        print("\nBenign samples should now have LOW susceptibility,", file=sys.stdout)
        print(
            "and ransomware samples should have HIGH susceptibility.", file=sys.stdout
        )
    else:
        print(
            f"\n[WARNING] {total - successful}/{total} register(s) failed to regenerate",
            file=sys.stderr,
        )
    ...
    if register_dir.exists():
        ...
        if register_files:
            print("  Generated register files:", file=sys.stdout)
