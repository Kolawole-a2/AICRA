...
def regenerate_diverse_learned_mapping(
    ...
    det_pairs = set(
        zip(det_df[det_tech_col].dropna(), det_df[det_ctrl_col].dropna())
    )
    learned_pairs = set(
        zip(learned_df["technique_id"].dropna(), learned_df["control_id"].dropna())
    )
    ...
    if len(only_in_det) == 0 and len(only_in_learned) == 0 and len(det_pairs) > 0:
        ...
        logger.error("=" * 80)
        logger.error("Even with top_k={top_k}, the learned mapping is identical to deterministic.")
        logger.error("This may indicate:")
        logger.error("1. Embedding model is producing identical similarity scores")
        logger.error("2. Deterministic pairs happen to match top-k embedding similarities")
