...
with open(log_file, "w", encoding="utf-8") as log:
    ...
    det_dac = [
        float(r["deterministic"]["mapping_metrics"]["dac_%"]) for r in all_results
    ]
    ...
    for f in plots_dir.glob("*.png"):
        if f.exists():
            ...
            f.unlink()
            log.write(
                f"  Deleted: {f.name} (was {old_stat.st_size} bytes, modified {datetime.datetime.fromtimestamp(old_stat.st_mtime)})\n"
            )
    log.write("\n")
    ...
    stat1 = f1.stat()
    log.write(
        f"  ✓ Saved: {f1.name} ({stat1.st_size:,} bytes, modified {datetime.datetime.fromtimestamp(stat1.st_mtime)})\n"
    )
    ...
    for bar, val in zip(bars1, det_precision, strict=False):
        ax.text(
            bar.get_x() + bar.get_width() / 2.0,
            bar.get_height() + 0.02,
            f"DET:{val:.3f}",
    ...
    ax.set_title(
        f"Actionable Precision Per Split - {timestamp}", fontsize=18, fontweight="bold"
    )
    ax.set_xticks(x)
    ax.set_xticklabels(splits, fontsize=14)
    ax.legend(fontsize=14)
    ax.grid(alpha=0.5, axis="y")
    y_max = (
        max(max(det_precision), max(learned_precision)) * 1.2
        if max(max(det_precision), max(learned_precision)) > 0
        else 1.2
    )
    ax.set_ylim([0, y_max])
    plt.tight_layout()
    ...
    stat2 = f2.stat()
    log.write(
        f"  ✓ Saved: {f2.name} ({stat2.st_size:,} bytes, modified {datetime.datetime.fromtimestamp(stat2.st_mtime)})\n"
    )
    ...
    for bar, val in zip(bars1, det_var_red, strict=False):
        ax.text(
            bar.get_x() + bar.get_width() / 2.0,
            bar.get_height(),
            f"DET:{val:.6f}",
    ...
    for bar, val in zip(bars2, learned_var_red, strict=False):
        ax.text(
            bar.get_x() + bar.get_width() / 2.0,
            bar.get_height(),
            f"LRN:{val:.6f}",
    ...
    ax.set_title(
        f"Variance Reduction Per Split - {timestamp}", fontsize=18, fontweight="bold"
    )
    ax.set_xticks(x)
    ax.set_xticklabels(splits, fontsize=14)
    ax.legend(fontsize=14)
    ...
    stat3 = f3.stat()
    log.write(
        f"  ✓ Saved: {f3.name} ({stat3.st_size:,} bytes, modified {datetime.datetime.fromtimestamp(stat3.st_mtime)})\n"
    )
    ...
    stat4 = f4.stat()
    log.write(
        f"  ✓ Saved: {f4.name} ({stat4.st_size:,} bytes, modified {datetime.datetime.fromtimestamp(stat4.st_mtime)})\n"
    )
