...
bars1 = ax.bar(x - width/2, det_dac_arr, width, 
               label="Deterministic", 
               color="#2e7d32", 
               alpha=0.9, 
               edgecolor='black', 
...
bars2 = ax.bar(x + width/2, learned_dac_arr, width, 
               label="Learned", 
               color="#1976d2", 
               alpha=0.9, 
               edgecolor='black', 
...
# Add value labels
for i, (bar, val) in enumerate(zip(bars1, det_dac_arr)):
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2., height + 1,
            f'{val:.1f}%', ha='center', va='bottom', 
            fontsize=11, fontweight='bold', color='darkgreen')

for i, (bar, val) in enumerate(zip(bars2, learned_dac_arr)):
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2., height + 1,
            f'{val:.1f}%', ha='center', va='bottom', 
...
bars1 = ax.bar(x - width/2, det_prec_arr, width, 
               label="Deterministic", color="#2e7d32", alpha=0.9, 
               edgecolor='black', linewidth=1.5)
bars2 = ax.bar(x + width/2, learned_prec_arr, width, 
               label="Learned", color="#1976d2", alpha=0.9, 
               edgecolor='black', linewidth=1.5)

for bar, val in zip(bars1, det_prec_arr):
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2., height + 0.02,
            f'{val:.3f}', ha='center', va='bottom', 
            fontsize=11, fontweight='bold', color='darkgreen')

for bar, val in zip(bars2, learned_prec_arr):
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2., height + 0.02,
            f'{val:.3f}', ha='center', va='bottom', 
            fontsize=11, fontweight='bold', color='darkblue')
...
ax.grid(alpha=0.3, axis='y')
ax.set_ylim([0, max(max(det_prec_arr), max(learned_prec_arr)) * 1.2 if max(max(det_prec_arr), max(learned_prec_arr)) > 0 else 1.2])
...
bars1 = ax.bar(x - width/2, det_var_arr, width, 
               label="Deterministic", color="#2e7d32", alpha=0.9, 
               edgecolor='black', linewidth=1.5)
bars2 = ax.bar(x + width/2, learned_var_arr, width, 
               label="Learned", color="#1976d2", alpha=0.9, 
               edgecolor='black', linewidth=1.5)

for bar, val in zip(bars1, det_var_arr):
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2., height,
            f'{val:.6f}', ha='center', va='bottom', 
            fontsize=9, color='darkgreen')

for bar, val in zip(bars2, learned_var_arr):
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2., height,
            f'{val:.6f}', ha='center', va='bottom', 
...
for ax, metric, det_mean, det_std, learned_mean, learned_std in zip(
    ...
    bars1 = ax.bar(0 - width/2, det_mean, width, yerr=det_std, 
                   label="Deterministic", color="#2e7d32", alpha=0.9, 
                   capsize=5, edgecolor='black', linewidth=1.5)
    bars2 = ax.bar(0 + width/2, learned_mean, width, yerr=learned_std, 
                   label="Learned", color="#1976d2", alpha=0.9, 
                   capsize=5, edgecolor='black', linewidth=1.5)
    
    # Value labels
    label_y1 = det_mean + det_std + (0.05 * det_mean if det_mean > 0 else 0.05)
    label_y2 = learned_mean + learned_std + (0.05 * learned_mean if learned_mean > 0 else 0.05)
    
    if metric == "DAC (%)":
        ax.text(0 - width/2, label_y1, f'{det_mean:.1f}%', 
                ha='center', va='bottom', fontsize=11, fontweight='bold', color='darkgreen')
        ax.text(0 + width/2, label_y2, f'{learned_mean:.1f}%', 
                ha='center', va='bottom', fontsize=11, fontweight='bold', color='darkblue')
    elif metric == "Actionable Precision":
        ax.text(0 - width/2, label_y1, f'{det_mean:.3f}', 
                ha='center', va='bottom', fontsize=11, fontweight='bold', color='darkgreen')
        ax.text(0 + width/2, label_y2, f'{learned_mean:.3f}', 
                ha='center', va='bottom', fontsize=11, fontweight='bold', color='darkblue')
    else:
        ax.text(0 - width/2, label_y1, f'{det_mean:.6f}', 
                ha='center', va='bottom', fontsize=9, color='darkgreen')
        ax.text(0 + width/2, label_y2, f'{learned_mean:.6f}', 
