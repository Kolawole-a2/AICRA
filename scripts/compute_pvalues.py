...
def compute_h1_pvalues(repo_root: Path) -> dict[str, Any]:
    ...
    t_stat_auroc, p_auroc_ttest = stats.ttest_1samp(
        auroc_values, auroc_threshold, alternative="greater"
    )
    ...
    logger.info(
        f"AUROC test (>=0.95): mean={auroc_mean:.4f}, threshold={auroc_threshold_strict:.4f}"
    )
    ...
    results = {
        ...
        "tests": {
            "auroc_vs_088": {
                ...
                "decision_alpha_005": (
                    "reject" if p_auroc_ttest < 0.05 else "fail_to_reject"
                ),
            },
            "auroc_vs_095": {
                ...
                "decision_alpha_005": (
                    "reject" if p_auroc_strict < 0.05 else "fail_to_reject"
                ),
            },
            "f1_vs_088": {
            ...
            "auroc_vs_random": {
                ...
                "decision_alpha_005": (
                    "reject" if auroc_vs_random_p < 0.05 else "fail_to_reject"
                ),
            },
        },
...
def compute_h2_pvalues(repo_root: Path) -> dict[str, Any]:
    ...
    f1_uncal_loss = [
        r["f1_optimized"]["uncalibrated"]["expected_loss"] for r in per_split_results
    ]
    f1_cal_loss = [
        r["f1_optimized"]["calibrated"]["expected_loss"] for r in per_split_results
    ]
    cost_uncal_loss = [
    ...
    t_stat_brier, p_brier = stats.ttest_rel(
        brier_uncal, brier_cal, alternative="greater"
    )

    # Wilcoxon signed-rank test (non-parametric)
    w_stat_brier, p_brier_wilcoxon = stats.wilcoxon(
        brier_uncal, brier_cal, alternative="greater"
    )

    logger.info(
        f"Brier improvement test: mean(diff)={brier_diff_mean:.6f}, std={brier_diff_std:.6f}"
    )
    ...
    # Wilcoxon signed-rank test (non-parametric)
    w_stat_ece, p_ece_wilcoxon = stats.wilcoxon(
        ece_uncal, ece_cal, alternative="greater"
    )

    logger.info(
        f"ECE improvement test: mean(diff)={ece_diff_mean:.6f}, std={ece_diff_std:.6f}"
    )
    ...
    t_stat_loss_uncal, p_loss_uncal = stats.ttest_rel(
        f1_uncal_loss, cost_uncal_loss, alternative="greater"
    )
    w_stat_loss_uncal, p_loss_uncal_wilcoxon = stats.wilcoxon(
        f1_uncal_loss, cost_uncal_loss, alternative="greater"
    )

    logger.info(
        f"Expected Loss (uncal) - Cost vs F1: mean(diff)={loss_diff_uncal_mean:.6f}, std={loss_diff_uncal_std:.6f}"
    )
    ...
    t_stat_loss_cal, p_loss_cal = stats.ttest_rel(
        f1_cal_loss, cost_cal_loss, alternative="greater"
    )
    w_stat_loss_cal, p_loss_cal_wilcoxon = stats.wilcoxon(
        f1_cal_loss, cost_cal_loss, alternative="greater"
    )

    logger.info(
        f"Expected Loss (cal) - Cost vs F1: mean(diff)={loss_diff_cal_mean:.6f}, std={loss_diff_cal_std:.6f}"
    )
    ...
    t_stat_cal_vs_uncal, p_cal_vs_uncal = stats.ttest_rel(
        cost_cal_loss, cost_uncal_loss, alternative="less"
    )
    w_stat_cal_vs_uncal, p_cal_vs_uncal_wilcoxon = stats.wilcoxon(
        cost_cal_loss, cost_uncal_loss, alternative="less"
    )

    logger.info(
        f"Expected Loss - Calibrated vs Uncalibrated (Cost-Optimized): mean(diff)={cal_vs_uncal_diff_mean:.6f}, std={cal_vs_uncal_diff_std:.6f}"
    )
    logger.info(f"  paired t-test p-value: {p_cal_vs_uncal:.6f}")
    logger.info(f"  Wilcoxon p-value: {p_cal_vs_uncal_wilcoxon:.6f}")
    logger.info(
        f"  Interpretation: Calibrated is {'better' if cal_vs_uncal_diff_mean < 0 else 'worse'} than uncalibrated"
    )
    ...
    results = {
        ...
        "tests": {
            "expected_loss_uncalibrated": {
                ...
                "decision_alpha_005": (
                    "reject" if p_loss_uncal < 0.05 else "fail_to_reject"
                ),
            ...
            "expected_loss_calibrated": {
                ...
                "decision_alpha_005": (
                    "reject" if p_loss_cal < 0.05 else "fail_to_reject"
                ),
                "note": "PRIMARY TEST: Cost-optimized thresholds reduce expected loss (decision-aligned metric)",
            },
            "brier_improvement": {
            ...
            "calibrated_vs_uncalibrated_cost_optimized": {
                ...
                "percent_change": (
                    float(100 * cal_vs_uncal_diff_mean / np.mean(cost_uncal_loss))
                    if np.mean(cost_uncal_loss) > 0
                    else 0.0
                ),
                ...
                "decision_alpha_005": (
                    "reject" if p_cal_vs_uncal < 0.05 else "fail_to_reject"
                ),
...
def compute_h3_pvalues(repo_root: Path) -> dict[str, Any]:
    ...
    # Load H3 results
    h3_results_path = (
        repo_root / "results" / "H3_full_evaluation" / "H3_full_results.json"
    )
    ...
    det_coverage = [
        r["deterministic"]["mapping_metrics"]["coverage_%"] for r in per_split_results
    ]
    learned_coverage = [
        r["learned"]["mapping_metrics"]["coverage_%"] for r in per_split_results
    ]
    det_dac = [
        r["deterministic"]["mapping_metrics"]["dac_%"] for r in per_split_results
    ]
    ...
    learned_precision = [
        r["learned"]["actionable_metrics"]["actionable_precision"]
        for r in per_split_results
    ]
    ...
    if coverage_diff_mean == 0.0 and np.all(coverage_diff == 0):
        logger.info(
            "Coverage test: Both mappings achieve 100% coverage (no difference)"
        )
        p_coverage = 1.0  # No difference to test
        p_coverage_wilcoxon = 1.0
    else:
        t_stat_coverage, p_coverage = stats.ttest_rel(
            det_coverage, learned_coverage, alternative="greater"
        )
        w_stat_coverage, p_coverage_wilcoxon = stats.wilcoxon(
            det_coverage, learned_coverage, alternative="greater"
        )
    ...
    if dac_diff_mean == 100.0 and np.all(dac_diff == 100.0):
        ...
        w_stat_dac, p_dac_wilcoxon = stats.wilcoxon(
            det_dac, learned_dac, alternative="greater"
        )
    ...
    valid_splits = [
        (d, lrn)
        for d, lrn in zip(det_precision, learned_precision, strict=False)
        if d > 0 or lrn > 0
    ]
    if len(valid_splits) > 1:
        det_prec_valid = [d for d, _lrn in valid_splits]
        learned_prec_valid = [lrn for _d, lrn in valid_splits]
        t_stat_precision, p_precision = stats.ttest_rel(
            det_prec_valid, learned_prec_valid, alternative="greater"
        )
        w_stat_precision, p_precision_wilcoxon = stats.wilcoxon(
            det_prec_valid, learned_prec_valid, alternative="greater"
        )
    else:
    ...
    results = {
        ...
        "tests": {
            "coverage": {
                ...
                "paired_ttest_pvalue": (
                    float(p_coverage) if p_coverage is not None else None
                ),
                "wilcoxon_pvalue": (
                    float(p_coverage_wilcoxon)
                    if p_coverage_wilcoxon is not None
                    else None
                ),
                "decision_alpha_005": (
                    "not_applicable"
                    if coverage_diff_mean == 0
                    else ("reject" if p_coverage < 0.05 else "fail_to_reject")
                ),
            ...
            "dac": {
                ...
                "wilcoxon_pvalue": (
                    float(p_dac_wilcoxon) if p_dac_wilcoxon is not None else None
                ),
            ...
            "precision": {
                ...
                "paired_ttest_pvalue": (
                    float(p_precision) if p_precision is not None else None
                ),
                "wilcoxon_pvalue": (
                    float(p_precision_wilcoxon)
                    if p_precision_wilcoxon is not None
                    else None
                ),
                "decision_alpha_005": (
                    "not_applicable"
                    if p_precision is None
                    else ("reject" if p_precision < 0.05 else "fail_to_reject")
                ),
...
def main():
    ...
    for hyp_name, hyp_results in all_results.items():
        ...
        for test_name, test_results in hyp_results.get("tests", {}).items():
            pval = test_results.get("ttest_pvalue") or test_results.get(
                "paired_ttest_pvalue"
            )
            decision = test_results.get("decision_alpha_005", "unknown")
            logger.info(f"  {test_name}:")
            logger.info(
                f"    p-value: {pval:.6f}" if pval is not None else "    p-value: N/A"
            )
