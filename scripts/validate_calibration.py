...
def main():
    ...
    print("-" * 80)
    print(f"Brier Score - Uncalibrated: {np.mean(brier_uncal):.6f} (std: {np.std(brier_uncal):.6f})")
    print(f"Brier Score - Calibrated:   {np.mean(brier_cal):.6f} (std: {np.std(brier_cal):.6f})")
    print(f"Brier Improvement: {np.mean(brier_uncal) - np.mean(brier_cal):.6f} ({100*(np.mean(brier_uncal) - np.mean(brier_cal))/np.mean(brier_uncal):.2f}%)")
    ...
    print(f"ECE Improvement: {np.mean(ece_uncal) - np.mean(ece_cal):.6f} ({100*(np.mean(ece_uncal) - np.mean(ece_cal))/np.mean(ece_uncal):.2f}%)")
    
    # Extract classification metrics at cost-optimized thresholds
    cost_uncal_precision = [r["cost_optimized"]["uncalibrated"]["precision"] for r in per_split_results]
    cost_uncal_recall = [r["cost_optimized"]["uncalibrated"]["recall"] for r in per_split_results]
    cost_uncal_f1 = [r["cost_optimized"]["uncalibrated"]["f1"] for r in per_split_results]
    
    cost_cal_precision = [r["cost_optimized"]["calibrated"]["precision"] for r in per_split_results]
    cost_cal_recall = [r["cost_optimized"]["calibrated"]["recall"] for r in per_split_results]
    ...
    print("-" * 80)
    print("Uncalibrated:")
    print(f"  Precision: {cost_uncal_precision} -> mean: {np.mean(cost_uncal_precision):.4f}")
    print(f"  Recall:    {cost_uncal_recall} -> mean: {np.mean(cost_uncal_recall):.4f}")
    print(f"  F1:        {cost_uncal_f1} -> mean: {np.mean(cost_uncal_f1):.4f}")
    print("\nCalibrated:")
    print(f"  Precision: {cost_cal_precision} -> mean: {np.mean(cost_cal_precision):.4f}")
    ...
    print(f"  {'Calibrated is BETTER' if np.mean(prec_diff) > 0 else 'Uncalibrated is BETTER'} (higher is better)")
    ...
    print(f"  {'Calibrated is BETTER' if np.mean(recall_diff) > 0 else 'Uncalibrated is BETTER'} (higher is better)")
    ...
    print(f"  {'Calibrated is BETTER' if np.mean(f1_diff) > 0 else 'Uncalibrated is BETTER'} (higher is better)")
    ...
    print(f"  {'Uncalibrated is BETTER' if np.mean(loss_diff) > 0 else 'Calibrated is BETTER'} (lower is better)")
    ...
    if np.mean(brier_cal) < np.mean(brier_uncal):
        print(f"  [OK] Brier Score improved: {np.mean(brier_uncal) - np.mean(brier_cal):.6f}")
    else:
        print(f"  [WARN] Brier Score worsened: {np.mean(brier_cal) - np.mean(brier_uncal):.6f}")
        print(f"    Reason: Model may already be well-calibrated (uncal Brier={np.mean(brier_uncal):.6f})")
    ...
    if np.mean(ece_cal) < np.mean(ece_uncal):
        ...
        print(f"    Reason: Calibration may be overfitting or model already well-calibrated")
    
    print("\n5. SUMMARY")
    print("-" * 80)
    print("For Risk Classification/Bucketing:")
    if np.mean(cost_cal_precision) > np.mean(cost_uncal_precision):
        print(f"  [OK] Calibrated has HIGHER precision: {np.mean(cost_cal_precision):.4f} vs {np.mean(cost_uncal_precision):.4f}")
    else:
        print(f"  [WARN] Calibrated has LOWER precision: {np.mean(cost_cal_precision):.4f} vs {np.mean(cost_uncal_precision):.4f}")
    
    if np.mean(cost_cal_recall) > np.mean(cost_uncal_recall):
        print(f"  [OK] Calibrated has HIGHER recall: {np.mean(cost_cal_recall):.4f} vs {np.mean(cost_uncal_recall):.4f}")
    else:
        print(f"  [WARN] Calibrated has LOWER recall: {np.mean(cost_cal_recall):.4f} vs {np.mean(cost_uncal_recall):.4f}")
    
    if np.mean(cost_cal_f1) > np.mean(cost_uncal_f1):
        print(f"  [OK] Calibrated has HIGHER F1: {np.mean(cost_cal_f1):.4f} vs {np.mean(cost_uncal_f1):.4f}")
    else:
        print(f"  [WARN] Calibrated has LOWER F1: {np.mean(cost_cal_f1):.4f} vs {np.mean(cost_uncal_f1):.4f}")
    
    print(f"\nFor Expected Loss (Cost-Weighted Performance):")
    if np.mean(cost_cal_loss) < np.mean(cost_uncal_loss):
        print(f"  [OK] Calibrated has LOWER expected loss: {np.mean(cost_cal_loss):.4f} vs {np.mean(cost_uncal_loss):.4f}")
    else:
        print(f"  [WARN] Calibrated has HIGHER expected loss: {np.mean(cost_cal_loss):.4f} vs {np.mean(cost_uncal_loss):.4f}")
        print(f"    This is the primary finding: calibration worsens cost-optimized performance")
