...
def extract_technique_id(attack_techniques_str: str) -> str | None:
    """
...
def convert_register_to_risk_scores(
    ...
    if missing:
        raise ValueError(
            f"Register file missing required columns: {missing}. Found: {list(df.columns)}"
        )
    ...
    if "attack_techniques" in df.columns:
        ...
        logger.warning(
            "No attack_techniques or technique_id column found. Setting technique_id to None."
        )
    ...
    assert_non_constant_scores(
        h3_df["risk_score"], split_name=split_name, min_unique=5, min_std=1e-6
    )
