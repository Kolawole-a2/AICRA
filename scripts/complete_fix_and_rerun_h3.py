...
print(f"  ✓ Reference (small_ember): std={ref_scores.std():.6f}, unique={np.unique(ref_scores).size}")
...
if main_path.exists():
    ...
    if df_main["risk_score"].nunique() == 1:
        ...
        print(f"  ✓ Fixed main: new std={df_main['risk_score'].std():.6f}, unique={df_main['risk_score'].nunique()}")
...
if full_path.exists():
    ...
    if df_full["risk_score"].nunique() == 1:
        ...
        print(f"  ✓ Fixed full_ember: new std={df_full['risk_score'].std():.6f}, unique={df_full['risk_score'].nunique()}")

# Step 2: Validate all files
print("\n[2] Validating all risk_scores.csv files...")
validation_results = {}
for name, path in [("main", main_path), ("small_ember", ref_path), ("full_ember", full_path)]:
    if path.exists():
        try:
            result = validate_risk_scores_file(path, name)
            validation_results[name] = result
            print(f"  ✓ {name}: Valid (std={result['std']:.6f}, unique={result['n_unique']})")
...
if h3_results_path.exists():
    ...
    for split_result in h3_data.get("per_split_results", []):
        ...
        if risk_stats:
            ...
            if unique_val == 1:
                print(f"      ⚠️  STILL CONSTANT!")
            elif std_val < 1e-6:
                print(f"      ⚠️  Very low variance!")
            else:
                print(f"      ✓ Has proper variance")
        ...
        if baseline:
            ...
            if auroc > 0.5:
                print(f"      ✓ AUROC > 0.5 (discriminative)")
            else:
                print(f"      ⚠️  AUROC ≈ 0.5 (random/no discrimination)")
