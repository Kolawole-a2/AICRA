...
def load_h1_results(results_dir: Path) -> Optional[Dict[str, Any]]:
...
def load_h2_results(results_dir: Path) -> Optional[Dict[str, Any]]:
    """Load H2 calibration and thresholding results."""
    # Try H2_full_results.json first, then metrics.json
    full_results_path = results_dir / "H2_calibration_thresholds" / "H2_full_results.json"
...
def load_h3_results(results_dir: Path) -> Optional[Dict[str, Any]]:
...
def calculate_percentage_improvement(
    ...
    if lower_is_better:
        ...
        if baseline_value == 0:
            ...
            relative_pct = -100.0 * (delta_absolute / baseline_value)  # Negative because reduction is improvement
    else:
...
def format_metric(value: float, metric_name: str) -> str:
    ...
    if "pct" in metric_name.lower() or "%" in metric_name or "dac" in metric_name.lower():
        return f"{value:.2f}%"
...
def generate_h1_section(h1_results: Dict[str, Any], baselines: Dict[str, float]) -> str:
    ...
    section += "**Hypothesis:** Static PE features enable reliable ransomware classification "
    ...
    auroc_improvement = calculate_percentage_improvement(aicra_auroc, baselines["auroc"])
    pr_auc_improvement = calculate_percentage_improvement(aicra_pr_auc, baselines["pr_auc"])
    brier_improvement = calculate_percentage_improvement(aicra_brier, baselines["brier_score"], lower_is_better=True)
    ece_improvement = calculate_percentage_improvement(aicra_ece, baselines["ece"], lower_is_better=True)
    precision_improvement = calculate_percentage_improvement(aicra_precision, baselines["precision"])
    recall_improvement = calculate_percentage_improvement(aicra_recall, baselines["recall"])
    ...
    section += f"### Primary Metric: AUROC\n\n"
    ...
    section += "indicating better discrimination between ransomware and benign samples. "
    ...
    section += "system with operational precision suitable for banking endpoint security.\n\n"
...
def generate_h2_section(h2_results: Dict[str, Any], baselines: Dict[str, float]) -> str:
    ...
    section += f"### Primary Metrics: Brier Score and ECE\n\n"
...
def generate_h3_section(h3_results: Dict[str, Any], baselines: Dict[str, float]) -> str:
    ...
    section += "### Key Metrics\n\n"
    section += "| Metric | Baseline (Naive) | Deterministic | Learned | Δ (Det - Learned) |\n"
    section += "|--------|------------------|--------------|---------|-------------------|\n"
    ...
    section += f"### Primary Metric: DAC_internal\n\n"
...
def generate_summary_table(
    h1_results: Optional[Dict[str, Any]],
    h2_results: Optional[Dict[str, Any]],
    h3_results: Optional[Dict[str, Any]],
    baselines: Dict[str, Dict[str, float]],
) -> str:
    """Generate summary table for all hypotheses."""
    table = "## Summary Table\n\n"
    table += "| Hypothesis | Metric(s) | Baseline | AICRA | Δ Absolute | Δ Relative (%) |\n"
    table += "|------------|-----------|----------|-------|------------|----------------|\n"
    
    # H1
    if h1_results:
        aicra_auroc = h1_results.get("auroc", 0.0)
        auroc_improvement = calculate_percentage_improvement(aicra_auroc, baselines["H1"]["auroc"])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Praxis Validation Report"
    )
    parser.add_argument(
        "--results-dir",
        type=Path,
    ...
    if h1_results:
        ...
        report += "**Status:** Results not available. Please run H1 experiment first.\n\n"
    
    if h2_results:
        report += generate_h2_section(h2_results, baselines["H2"])
    else:
        report += "## H2: Risk Calibration / Risk Scoring Stability\n\n"
        report += "**Status:** Results not available. Please run H2 experiment first.\n\n"
    ...
    if h3_results:
        ...
        report += "**Status:** Results not available. Please run H3 experiment first.\n\n"
