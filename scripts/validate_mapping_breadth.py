...
def main():
    ...
    det_pairs = set(
        zip(det_df[det_col].astype(str), det_df[det_ctrl_col].astype(str))
    )
    ...
    learned_pairs = set(
        zip(learned_df["technique_id"].astype(str), learned_df["control_id"].astype(str))
    )
    ...
    if len(learned_only_pairs) == 0:
        print("❌ FAIL: Learned mapping contains NO controls that are not in deterministic")
        sys.exit(1)
    else:
    ...
    print()
    print(f"Techniques with only deterministic controls: {len(techniques_with_only_det_controls)}/{len(learned_by_tech)}")
    
    if len(techniques_with_only_det_controls) == len(learned_by_tech):
        print("❌ FAIL: For ALL techniques, learned controls are subsets of deterministic")
        sys.exit(1)
    else:
    ...
    print()
    print("Summary:")
    print(f"  - Learned pairs: {len(learned_pairs)} (vs {len(det_pairs)} deterministic)")
    print(f"  - Learned-only pairs: {len(learned_only_pairs)}")
    print(f"  - Techniques with extra learned controls: {len(learned_by_tech) - len(techniques_with_only_det_controls)}/{len(learned_by_tech)}")
    print()
