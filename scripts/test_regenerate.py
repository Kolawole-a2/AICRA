...
if learned_path.exists():
    ...
    if det_path.exists():
        ...
        det_pairs = set(
            zip(
                det_df[det_col].astype(str),
                det_df[det_ctrl_col].astype(str),
                strict=False,
            )
        )
        learned_pairs = set(
            zip(
                df["technique_id"].astype(str),
        ...
        print(
            f"  Is broader: {len(learned_pairs) > len(det_pairs) and len(learned_pairs - det_pairs) > 0}"
        )
else:
