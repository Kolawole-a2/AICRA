...
def main():
    ...
    # Extract pairs
    det_pairs = set(zip(det_df["technique_id"], det_df["control_id"], strict=False))
    lrn_pairs = set(zip(lrn_df["technique_id"], lrn_df["control_id"], strict=False))
    ...
    print(
        f"{'Avg pairs per technique':<30} {len(det_pairs) / len(det_techs):<20.2f} {len(lrn_pairs) / len(lrn_techs):<20.2f}"
    )
    ...
    print(
        f"Overlap:                 {len(tech_overlap)} ({len(tech_overlap) / len(det_techs) * 100:.1f}%)"
    )
    ...
    print(
        f"Overlap:               {len(ctrl_overlap)} ({len(ctrl_overlap) / len(det_ctrls) * 100:.1f}% of deterministic)"
    )
    ...
    if lrn_only_ctrls:
        print(
            f"  Controls only in learned: {len(lrn_only_ctrls)} (showing first 10: {sorted(lrn_only_ctrls)[:10]})"
        )
    ...
    print(
        f"Overlapping pairs:          {len(pair_overlap)} ({len(pair_overlap) / len(det_pairs) * 100:.1f}% of deterministic)"
    )
    ...
    if len(pair_overlap) == 0:
        ...
        print(
            f"✓ Mappings are sufficiently different ({len(pair_overlap) / len(det_pairs) * 100:.1f}% overlap)"
        )
    else:
        print(
            f"⚠ Mappings have significant overlap ({len(pair_overlap) / len(det_pairs) * 100:.1f}%)"
        )
    ...
    print(
        f"Deterministic self-consistency: {det_dac:.2f}% ({len(pair_overlap)} / {len(det_pairs)})"
    )
    print(
        f"Learned consistency vs deterministic: {lrn_dac:.2f}% ({len(pair_overlap)} / {len(lrn_pairs)})"
    )
    ...
    print("Techniques with mappings:")
