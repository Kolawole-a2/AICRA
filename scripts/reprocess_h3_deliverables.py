...
def regenerate_learned_mapping():
    ...
    cmd = [
        ...
        "--top-k", "10",
        "--min-similarity", "0.25",
        "--out", str(repo_root / "data" / "mappings" / "learned_mapping.csv"),
    ]
    ...
    if "--attack" not in " ".join(cmd) or "--d3fend" not in " ".join(cmd):
        ...
        LOGGER.error("  python -m aicra.mappings.heuristic_mapping --attack <path> --d3fend <path> --top-k 10 --min-similarity 0.25 --out data/mappings/learned_mapping.csv")
        return False
    ...
    if result.returncode != 0:
        LOGGER.error(f"Failed to regenerate learned mapping:")
...
def run_h3_evaluation():
    ...
    cmd = [
        ...
        "--config", str(repo_root / "config" / "h3_splits.yaml"),
        "--deterministic", str(repo_root / "data" / "mappings" / "deterministic_attack_defense_lookup.csv"),
        "--learned", str(repo_root / "data" / "mappings" / "learned_mapping.csv"),
        "--output", str(repo_root / "results" / "H3_full_evaluation"),
    ]
    ...
    if result.returncode != 0:
        LOGGER.error(f"H3 evaluation failed:")
        LOGGER.error(result.stderr)
        return False
