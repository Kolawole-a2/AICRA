...
for f in plots_dir.glob("*.png"):
    ...
    f.unlink()
    print(f"  Deleted: {f.name} (was modified: {datetime.datetime.fromtimestamp(old_time)})")
...
bars1 = ax.bar(x - width/2, det_dac, width, 
...
bars2 = ax.bar(x + width/2, learned_dac, width, 
               label="Learned (BLUE)", 
               color="blue", 
               alpha=1.0, 
               edgecolor='black', 
