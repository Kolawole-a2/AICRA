...
**Evaluation & Metrics:**
- `aicra/experiments/h1_classification.py` - H1 experiment with AUROC, PR-AUC, Brier, ECE, Lift@k
...
- `aicra/pipelines/training.py` - Training pipeline with bagging (seeds parameter)
...
**Risk Bucketing & Controls:**
- `aicra/register.py` - Risk register generation with bucketing (0.33, 0.66 thresholds)
...
**Banking Narratives & Policy:**
- `aicra/pipelines/policy.py` - PolicyPipeline with cost-sensitive threshold optimization
...
**Data Loading & Splits:**
- `aicra/utils/data_loader.py` - Data loading (no explicit time-ordered split in main function)
...
**✅ Time-Ordered Split:**
- **Location:** `aicra/pipelines/evaluation.py`, lines 176-187
  ```python
...
- **Location:** `aicra/experiments/h1_classification.py`, lines 191-214
  ```python
...
**✅ Metrics Coverage:**
- **AUROC:** ✅ `aicra/experiments/h1_classification.py:173`, `aicra/core/evaluation.py:44`
...
**⚠️ Operational Decision Threshold (FN≫FP):**
- **Location:** `aicra/core/evaluation.py`, lines 54-67 (`cost_sensitive_threshold()`)
...
**✅ Calibration Checks:**
- **Location:** `aicra/pipelines/calibration.py`, lines 27-82
  ```python
...
- **Focal Loss:** `aicra/pipelines/training.py:202-220`
  ```python
  class FocalLoss:
...
**✅ Risk Bucketing:**
- **Location:** `aicra/register.py`, lines 60-61
  ```python
...
**✅ Control Definitions:**
- **Location:** `data/lookups/risk_bucket_controls.yaml`
  ```yaml
  High:
    controls:
...
- **Location:** `aicra/pipelines/mapping.py` - `MappingPipeline` class
...
**⚠️ Binding Buckets to Controls:**
- **Location:** `aicra/utils/policy_writer.py:77-79` - Computes expected_loss but does **NOT** attach controls to buckets
...
**⚠️ ATT&CK-Informed Playbooks:**
- **Location:** `aicra/utils/policy_writer.py:83-84` - Writes register JSON but does **NOT** generate playbooks
...
- **Location:** `aicra/utils/policy_writer.py:77`
  ```python
...
- **Location:** `aicra/core/evaluation.py:54-67`
  ```python
...
**✅ Policy JSON Persistence:**
- **Location:** `aicra/pipelines/policy.py:130-180`
  ```python
  @dataclass
  class Policy:
      threshold: float
...
**⚠️ Impact Parameterization:**
- **Issue:** `impact_default` is a single value, not parameterized per:
...
**⚠️ Banking-Specific Parameters:**
- **Issue:** Policy JSON does not include:
...
- [ ] **Time-Ordered Split:**
  - [ ] Run: `python -m aicra.experiments.h1_classification --output results/H1_test --time-ordered`
...
- [ ] **Out-of-Family Test:**
  - [ ] Run: `python -m aicra.experiments.h1_classification --output results/H1_test`
...
- [ ] **FN≫FP Threshold:**
  - [ ] Run: `python -m aicra.experiments.h2_calibration_thresholds --cost-fn 100.0 --cost-fp 1.0`
...
- [ ] **Bagged Models:**
  - [ ] Check: `config/h1_config.yaml` - verify `seeds: 5`
...
- [ ] **Robust Loss:**
  - [ ] Check: `aicra/pipelines/training.py:154` - verify `FocalLoss(alpha=0.75, gamma=2.0)`
...
- [ ] **Risk Buckets:**
  - [ ] Run: `python -m aicra.register` (if exists) or check `register/risk_register_main.json`
  - [ ] Check: Verify `susceptibility_bucket` column has "High", "Medium", "Low"
  - [ ] Check: Verify thresholds are 0.33 and 0.66

- [ ] **Controls Binding:**
  - [ ] Check: `data/lookups/risk_bucket_controls.yaml` exists
...
- [ ] **ATT&CK→D3FEND Mapping:**
  - [ ] Check: `deterministic_lookup.csv` exists with `technique_id,control_id` columns
...
- [ ] **Playbook Generation:**
  - [ ] **Gap:** No playbook generation script found - needs implementation

### Requirement 4: Banking Narratives

- [ ] **Expected Loss:**
  - [ ] Check: `register/risk_register_main.json` - verify `expected_loss` column exists
...
- [ ] **Policy JSON:**
  - [ ] Check: `policies/policy.json` exists
