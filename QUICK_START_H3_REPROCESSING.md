...
**Or with PowerShell line continuation (backticks):**
```powershell
...
**Important Notes:**
- Use `aicra.mappings.heuristic_mapping` (with 's') - NOT `aicra.mapping.heuristic_mapping`
...
**Error: ModuleNotFoundError for 'aicra.mapping'**
→ Use `aicra.mappings` (with 's') instead

**Error: PowerShell syntax error with backslashes**
→ Use backticks (`) or put command on one line

**Error: Sanity check fails**
→ Increase `--top-k` (e.g., 12) or decrease `--min-similarity` (e.g., 0.20)
