...
**Problem:**
- `d3fend_reference_pairs.csv` had the same SHA256 hash as `deterministic_lookup.csv`
...
**Solution:**
- Created `scripts/create_reference_pairs.py` to generate canonical reference pairs from `data/lookups/attack_to_d3fend.yaml`
...
**Problem:**
- No validation that reference_pairs ≠ deterministic_mapping
- No validation that learned_mapping ≠ deterministic_mapping
- Experiments could run with invalid configurations

**Solution:**
- Added three sanity checks in `aicra/experiments/h3_evaluation.py`:
...
**Problem:**
- No dedicated H1 experiment file for static PE classification
- Training and evaluation were scattered across multiple files

**Solution:**
- Created `aicra/experiments/h1_classification.py`:
...
**Problem:**
- No dedicated H2 experiment file for calibration and thresholding
- Calibration and threshold optimization were not integrated

**Solution:**
- Created `aicra/experiments/h2_calibration_thresholds.py`:
...
**Problem:**
- No single script to run all hypothesis experiments
- Difficult to reproduce complete experimental pipeline

**Solution:**
- Created `scripts/run_all_hypotheses.py`:
