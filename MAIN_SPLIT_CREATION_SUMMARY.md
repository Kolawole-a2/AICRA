...
3. **Technique ID Extraction:**
   - Extracts first technique from `attack_techniques` column
   - Handles empty/missing values by defaulting to 'T1486'
   - Uses `ast.literal_eval()` to parse Python list strings
4. **H3 Format Columns:**
   - `asset_id`: Generated as `asset_0000`, `asset_0001`, etc.
...
2. **Expected Result:**
   - Main split should now be included in `splits_evaluated`
