...
def load_deterministic_mapping(
    ...
    logger.info(
        f"Loaded {len(mappings)} technique mappings with {sum(len(v) for v in mappings.values())} total pairs"
    )

    return mappings


def load_learned_mapping(mapping_path: str | Path) -> pd.DataFrame:
    """
    ...
    logger.info(
        f"Loaded {len(df)} learned mapping pairs covering {df['technique_id'].nunique()} techniques"
    )

    return df


def load_reference_pairs(ref_path: Path | None = None) -> pd.DataFrame:
    """
...
def compute_dac(
    mapping_df: pd.DataFrame, reference_df: pd.DataFrame, mapping_type: str = "unknown"
) -> float:
    """
...
def compute_coverage(
    mapping_df: pd.DataFrame, ransomware_techniques: list[str] | None = None
) -> float:
    """
    ...
    if ransomware_techniques is None:
        ...
        techniques_with_controls = mapping_df.dropna(subset=["control_id"])[
            "technique_id"
        ].nunique()
    else:
        techniques_total = len(set(ransomware_techniques))
        mapped_techniques = set(
            mapping_df.dropna(subset=["control_id"])["technique_id"].unique()
        )
...
def compute_dac_metrics(
    deterministic_mapping: dict[str, list[str]] | pd.DataFrame,
    learned_mapping: pd.DataFrame,
    reference_pairs: pd.DataFrame,
    ransomware_techniques: list[str] | None = None,
) -> dict[str, float]:
    """
    ...
    if isinstance(deterministic_mapping, dict):
        ...
        for technique_id, controls in deterministic_mapping.items():
            for control_id in controls:
                det_rows.append(
                    {"technique_id": technique_id, "control_id": control_id}
                )
    ...
    dac_det = compute_dac(
        det_df, reference_df=reference_pairs, mapping_type="deterministic"
    )
    dac_learned = compute_dac(
        learned_mapping, reference_df=reference_pairs, mapping_type="learned"
    )
...
def compute_dac_between_mappings(
    ...
    if missing_det:
        raise ValueError(
            f"Deterministic mapping missing required columns: {missing_det}"
        )
    if missing_learned:
    ...
    # Create pair strings
    df_det_clean["pair"] = (
        df_det_clean["attack_id"].astype(str)
        + "||"
        + df_det_clean["defense_id"].astype(str)
    )
    df_learned_clean["pair"] = (
        df_learned_clean["attack_id"].astype(str)
        + "||"
        + df_learned_clean["defense_id"].astype(str)
    )
...
def compute_dac_per_attack(
    ...
    all_attack_ids = set(df_det_clean["attack_id"].unique()) | set(
        df_learned_clean["attack_id"].unique()
    )
    ...
    for attack_id in all_attack_ids:
        ...
        det_pairs = set(
            df_det_clean[df_det_clean["attack_id"] == attack_id]["pair"].unique()
        )
        learned_pairs = set(
            df_learned_clean[df_learned_clean["attack_id"] == attack_id][
                "pair"
            ].unique()
        )
