...
def load_h1_results(results_dir: Path) -> Optional[Dict]:
...
def load_h2_results(results_dir: Path) -> Optional[Dict]:
...
def load_h3_results(results_dir: Path) -> Optional[Dict]:
...
def generate_benchmark_improvements_table(
    h1_results_dir: Optional[Path] = None,
    h2_results_dir: Optional[Path] = None,
    h3_results_dir: Optional[Path] = None,
    ...
    if h1_results and "improvement" in h1_results:
        ...
        rows.append({
            "hypothesis": "H1",
            "metric": "AUROC",
            "baseline_value": h1_results.get("baseline", {}).get("best_baseline", {}).get("auroc", "N/A"),
        ...
        rows.append({
            "hypothesis": "H1",
            "metric": "Precision",
            "baseline_value": h1_results.get("baseline", {}).get("best_baseline", {}).get("precision", "N/A"),
        ...
        rows.append({
            "hypothesis": "H1",
            "metric": "Recall",
            "baseline_value": h1_results.get("baseline", {}).get("best_baseline", {}).get("recall", "N/A"),
        ...
        rows.append({
            "hypothesis": "H1",
            "metric": "F1",
            "baseline_value": h1_results.get("baseline", {}).get("best_baseline", {}).get("f1", "N/A"),
        ...
        if "alert_fatigue_reduction" in h1_results:
            ...
            rows.append({
                ...
                "improvement_pct": afr.get("estimated_analyst_fatigue_reduction_pct", "N/A"),
    ...
    if h2_results and "improvement" in h2_results:
        ...
        rows.append({
            ...
            "aicra_value": h2_results.get("calibration", {}).get("brier_calibrated", "N/A"),
        ...
        rows.append({
            ...
            "aicra_value": h2_results.get("calibration", {}).get("ece_calibrated", "N/A"),
    ...
    if h3_results and "aggregated_metrics" in h3_results:
        ...
        if "improvements" in agg:
            ...
            rows.append({
                ...
                "baseline_value": "N/A",
                "aicra_value": "N/A",
                "improvement_pct": imp.get("estimated_fatigue_reduction_pct", "N/A"),
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write("This document summarizes the % improvements achieved by AICRA over baseline methods.\n\n")
        f.write("## H1: Static PE Classification Reliability\n\n")
        f.write("**Hypothesis**: LightGBM on EMBER-2024 static PE features predicts ransomware susceptibility.\n\n")
        ...
        if len(h1_rows) > 0:
            ...
            for _, row in h1_rows.iterrows():
                ...
                f.write(f"| {row['metric']} | {baseline} | {aicra} | {improvement_str} |\n")
            f.write("\n")
        
        f.write("## H2: Calibration and Transferability\n\n")
        f.write("**Hypothesis**: Isotonic calibration improves susceptibility score transferability.\n\n")
        ...
        f.write("**Hypothesis**: Deterministic ATT&CK–D3FEND lookup beats learned mapping.\n\n")
        
        h3_rows = df[df["hypothesis"] == "H3"]
        if len(h3_rows) > 0:
            f.write("| Metric | Baseline (Learned) | AICRA (Deterministic) | Improvement |\n")
            f.write("|--------|-------------------|----------------------|-------------|\n")
