...
def normalize_technique_id(tech_id: str) -> str | None:
    """
...
def validate_technique_id(
    tech_id: str, valid_techniques: set[str] | None = None
) -> tuple[bool, str | None]:
    """
...
def validate_technique_column(
    ...
    if drop_invalid:
        ...
        logger.info(
            f"Dropped {diagnostics['invalid_rows']} rows with invalid technique IDs"
        )

    return df_validated, diagnostics


def extract_valid_techniques_from_mapping(
    mapping_df: pd.DataFrame, technique_col: str = "technique_id"
) -> set[str]:
    """
...
def validate_risk_scores_file(
    file_path: str, valid_techniques: set[str] | None = None, drop_invalid: bool = False
) -> tuple[pd.DataFrame, dict]:
    """
