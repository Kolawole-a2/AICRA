...
import pandas as pd
import yaml
from scipy.stats import ttest_rel, wilcoxon, ks_2samp, mannwhitneyu, spearmanr
...
def load_mapping_csv(
    ...
    if missing:
        raise ValueError(f"Mapping file missing required columns: {missing}. Found: {list(df.columns)}")
...
def load_risk_scores(
    ...
    required = {"asset_id", "risk_score", "predicted_label", "true_label", "technique_id"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Risk scores file missing required columns: {missing}. Found: {list(df.columns)}")
    ...
    if validate_techniques and "technique_id" in df.columns:
        ...
        if "technique_id" in df.columns:
            ...
            diagnostics["unique_valid_techniques"] = int(df[valid_mask]["technique_id"].nunique())
...
def compute_dac(
    ...
    det_pairs = set(
        tuple(row) for row in deterministic_df[["technique_id", "control_id"]].dropna().values.tolist()
    )
...
def compute_dac_external(
    ...
    ref_pairs = set(
        tuple(row) for row in ref_pairs_df[["technique_id", "control_id"]].dropna().values.tolist()
    )
...
def compute_mapping_metrics(
    ...
    if len(all_techniques) == 0:
        logger.warning(f"compute_mapping_metrics: Split has 0 techniques - coverage will be 0%")
    else:
        logger.debug(f"compute_mapping_metrics: Split has {len(all_techniques)} unique techniques")
    ...
    if "validated" in mapping_df.columns:
        validated_pairs = mapping_df["validated"].notna() & (mapping_df["validated"] == 1)
        if len(mapping_df) > 0:
...
def compute_actionable_metrics(
    ...
    det_pairs_set = set(
        ...
        for _, row in deterministic_mapping[["technique_id", "control_id"]].dropna().iterrows()
    )
    ...
    for _, row in positives.iterrows():
        ...
        if is_deterministic:
            ...
            has_ransomware_relevant = any(
                (tech_str, str(ctrl)) in det_pairs_set 
...
def compute_score_consistency(
    ...
    baseline_iqr = float(np.percentile(risk_df["risk_score"], 75) - np.percentile(risk_df["risk_score"], 25))
    
    # Compute adjusted metrics
    adjusted_var = float(np.var(risk_df["risk_score_adjusted"], ddof=1))
    adjusted_iqr = float(np.percentile(risk_df["risk_score_adjusted"], 75) - np.percentile(risk_df["risk_score_adjusted"], 25))
...
def validate_mapping_results(
    ...
    diagnostics = {
        ...
        "variance_change_pct": ((mapped_variance - baseline_variance) / baseline_variance * 100) if baseline_variance > 0 else 0.0,
        "iqr_change_pct": ((mapped_iqr - baseline_iqr) / baseline_iqr * 100) if baseline_iqr > 0 else 0.0,
        "warnings": [],
        "contradictions": [],
    }
...
def compute_mapping_interpretation(
    ...
    baseline_actionable_precision: Optional[float] = None
) -> Dict:
    """
    ...
    if baseline_actionable_precision is None:
        ...
        baseline_precision = mapped_precision  # Conservative: assume no change if not provided
    else:
    ...
    try:
        if len(baseline_scores) > 0 and len(mapped_scores) > 0 and not np.array_equal(baseline_scores, mapped_scores):
            ks_result = ks_2samp(baseline_scores, mapped_scores)
            ks_statistic = float(ks_result.statistic)
            ks_pvalue = float(ks_result.pvalue)
            
            mw_result = mannwhitneyu(baseline_scores, mapped_scores, alternative='two-sided')
            mw_statistic = float(mw_result.statistic)
            mw_pvalue = float(mw_result.pvalue)
            
            significant_shift = (ks_pvalue is not None and ks_pvalue < 0.05) or (mw_pvalue is not None and mw_pvalue < 0.05)
    except Exception as e:
        logger.warning(f"Could not compute statistical tests: {e}")
    
    # Interpretation strings
    variance_interpretation = "increased" if validation["variance_increased"] else "decreased"
    if abs(validation["variance_change_pct"]) > 50.0:
        variance_interpretation += f" by {abs(validation['variance_change_pct']):.1f}% (large change)"
    else:
    ...
    if validation["variance_increased"]:
        ...
        variance_interpretation += ". Decreased variance indicates more consistent risk scores."
    
    iqr_interpretation = "increased" if validation["iqr_increased"] else "decreased"
    if abs(validation["iqr_change_pct"]) > 50.0:
        iqr_interpretation += f" by {abs(validation['iqr_change_pct']):.1f}% (large change)"
    else:
        iqr_interpretation += f" by {abs(validation['iqr_change_pct']):.1f}%"
    
    if validation["iqr_increased"]:
        iqr_interpretation += ". Increased IQR may indicate better separation between risk quartiles."
    else:
        iqr_interpretation += ". Decreased IQR indicates more compact risk score distribution."
...
def create_diagnostic_plots(
    ...
    ax.hist(baseline_scores, bins=50, alpha=0.6, label="Baseline", density=True, color="#1976d2")
    ax.hist(mapped_scores, bins=50, alpha=0.6, label="Mapped", density=True, color="#2e7d32")
    ax.set_xlabel("Risk Score", fontsize=12)
    ax.set_ylabel("Density", fontsize=12)
    ax.set_title(f"Risk Score Distribution: Baseline vs Mapped ({split_name})", fontsize=14, fontweight='bold')
    ax.legend(fontsize=11)
    ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(diagnostics_dir / f"distribution_density_{split_name}.png", dpi=150, bbox_inches='tight')
    plt.close()
    ...
    ax.set_title(f"Risk Score Distribution Comparison ({split_name})", fontsize=14, fontweight='bold')
    ax.grid(alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig(diagnostics_dir / f"distribution_boxplot_{split_name}.png", dpi=150, bbox_inches='tight')
    plt.close()
    ...
    ax.plot([min_score, max_score], [min_score, max_score], 'r--', alpha=0.5, label="y=x")
    ax.set_xlabel("Baseline Risk Score", fontsize=12)
    ax.set_ylabel("Mapped Risk Score", fontsize=12)
    ax.set_title(f"Baseline vs Mapped Risk Scores ({split_name})", fontsize=14, fontweight='bold')
    ax.legend(fontsize=11)
    ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(diagnostics_dir / f"distribution_scatter_{split_name}.png", dpi=150, bbox_inches='tight')
    plt.close()
...
def evaluate_split(
    ...
    if "technique_id" in risk_df.columns:
        ...
        logger.info(f"  Split '{split_name}': {len(risk_df)} samples, {n_samples_with_tech} with technique_id, {n_techniques} unique techniques")
        
        if n_techniques == 0:
            logger.warning(f"  ⚠️  WARNING: Split '{split_name}' has 0 unique techniques after validation - SKIPPING")
            logger.warning(f"     This split will not be included in metric aggregation or statistical tests")
            return None
        
        if n_samples_with_tech == 0:
            logger.warning(f"  ⚠️  WARNING: Split '{split_name}' has 0 samples with valid technique IDs - SKIPPING")
            return None
    
    # Compute mapping metrics (with deterministic as ground truth for DAC)
    det_mapping_metrics = compute_mapping_metrics(det_mapping, risk_df, ref_pairs, deterministic_df=det_mapping)
    learned_mapping_metrics = compute_mapping_metrics(learned_mapping, risk_df, ref_pairs, deterministic_df=det_mapping)
    ...
    learned_actionable = compute_actionable_metrics(risk_df, learned_mapping, det_mapping)
    ...
    # Compute deltas
    delta_dac = det_mapping_metrics.get("dac_%", 100.0) - learned_mapping_metrics.get("dac_%", 0.0)
    delta_coverage = det_mapping_metrics["coverage_%"] - learned_mapping_metrics["coverage_%"]
    delta_precision = det_actionable["actionable_precision"] - learned_actionable["actionable_precision"]
    delta_f1 = det_actionable["actionable_f1"] - learned_actionable["actionable_f1"]
    delta_variance_reduction = det_consistency["variance_reduction"] - learned_consistency["variance_reduction"]
    delta_iqr_reduction = det_consistency["iqr_reduction"] - learned_consistency["iqr_reduction"]
    ...
    baseline_actionable_precision = float(precision_score(y_true, y_pred, zero_division=0)) if len(y_true) > 0 else 0.0
...
def aggregate_metrics(all_results: List[Dict]) -> Dict:
    ...
    det_dac = [r["deterministic"]["mapping_metrics"].get("dac_%", 100.0) for r in all_results]
    learned_dac = [r["learned"]["mapping_metrics"].get("dac_%", 0.0) for r in all_results]
    det_coverage = [r["deterministic"]["mapping_metrics"]["coverage_%"] for r in all_results]
    learned_coverage = [r["learned"]["mapping_metrics"]["coverage_%"] for r in all_results]
    ...
    det_f1 = [r["deterministic"]["actionable_metrics"]["actionable_f1"] for r in all_results]
    learned_f1 = [r["learned"]["actionable_metrics"]["actionable_f1"] for r in all_results]
    ...
    det_iqr_red = [r["deterministic"]["consistency_metrics"]["iqr_reduction"] for r in all_results]
    learned_iqr_red = [r["learned"]["consistency_metrics"]["iqr_reduction"] for r in all_results]
    ...
    aggregated = {
        "deterministic": {
            "dac_%": {"mean": float(np.mean(det_dac)), "std": float(np.std(det_dac, ddof=1))},
            "coverage_%": {"mean": float(np.mean(det_coverage)), "std": float(np.std(det_coverage, ddof=1))},
            "actionable_precision": {"mean": float(np.mean(det_precision)), "std": float(np.std(det_precision, ddof=1))},
            "actionable_f1": {"mean": float(np.mean(det_f1)), "std": float(np.std(det_f1, ddof=1))},
            "variance_reduction": {"mean": float(np.mean(det_var_red)), "std": float(np.std(det_var_red, ddof=1))},
            "iqr_reduction": {"mean": float(np.mean(det_iqr_red)), "std": float(np.std(det_iqr_red, ddof=1))},
        },
        "learned": {
            "dac_%": {"mean": float(np.mean(learned_dac)), "std": float(np.std(learned_dac, ddof=1))},
            "coverage_%": {"mean": float(np.mean(learned_coverage)), "std": float(np.std(learned_coverage, ddof=1))},
            "actionable_precision": {"mean": float(np.mean(learned_precision)), "std": float(np.std(learned_precision, ddof=1))},
            "actionable_f1": {"mean": float(np.mean(learned_f1)), "std": float(np.std(learned_f1, ddof=1))},
            "variance_reduction": {"mean": float(np.mean(learned_var_red)), "std": float(np.std(learned_var_red, ddof=1))},
            "iqr_reduction": {"mean": float(np.mean(learned_iqr_red)), "std": float(np.std(learned_iqr_red, ddof=1))},
        },
        "deltas": {
            "delta_dac_%": {
    ...
    try:
        tstat_precision, pvalue_precision_ttest = ttest_rel(det_precision, learned_precision)
        try:
            wstat_precision, pvalue_precision_wilcoxon = wilcoxon(det_precision, learned_precision, alternative="two-sided")
        except ValueError:
    ...
    try:
        ...
        try:
            wstat_var_red, pvalue_var_red_wilcoxon = wilcoxon(det_var_red, learned_var_red, alternative="two-sided")
        except ValueError:
    ...
    if len(all_results) >= 3:
        ...
        if learned_dac_std > 1e-10:  # Has variation
            try:
                ...
                logger.warning(f"Spearman correlation (DAC vs precision, learned) failed: {e}")
        else:
            logger.info("DAC is constant across splits for learned mapping - Spearman correlation undefined")
        ...
        if learned_dac_std > 1e-10:
            try:
                ...
                logger.warning(f"Spearman correlation (DAC vs variance reduction, learned) failed: {e}")
    else:
        logger.warning(f"Too few splits ({len(all_results)}) for Spearman correlation (need ≥3)")
    
    aggregated["statistical_tests"] = {
        "dac": {
            "ttest": {
                "statistic": float(tstat_dac) if tstat_dac is not None else None,
                "pvalue": float(pvalue_dac_ttest) if pvalue_dac_ttest is not None else None,
                "note": "Tests learned DAC vs deterministic baseline (100%)",
            },
            "wilcoxon": {
                "statistic": float(wstat_dac) if wstat_dac is not None else None,
                "pvalue": float(pvalue_dac_wilcoxon) if pvalue_dac_wilcoxon is not None else None,
            },
            "spearman_vs_precision": {
                "rho_learned": float(rho_dac_prec_learned) if rho_dac_prec_learned is not None else None,
                "pvalue_learned": float(pval_dac_prec_learned) if pval_dac_prec_learned is not None else None,
                "note": "Undefined if DAC is constant across splits" if (learned_dac_std <= 1e-10) else None,
            },
            "spearman_vs_variance_reduction": {
                "rho_learned": float(rho_dac_var_learned) if rho_dac_var_learned is not None else None,
                "pvalue_learned": float(pval_dac_var_learned) if pval_dac_var_learned is not None else None,
                "note": "Undefined if DAC is constant across splits" if (learned_dac_std <= 1e-10) else None,
            },
        },
        "actionable_precision": {
            "ttest": {
                "statistic": float(tstat_precision) if tstat_precision is not None else None,
                "pvalue": float(pvalue_precision_ttest) if pvalue_precision_ttest is not None else None,
            },
            "wilcoxon": {
                "statistic": float(wstat_precision) if wstat_precision is not None else None,
                "pvalue": float(pvalue_precision_wilcoxon) if pvalue_precision_wilcoxon is not None else None,
            },
        },
        "variance_reduction": {
            "ttest": {
                "statistic": float(tstat_var_red) if tstat_var_red is not None else None,
                "pvalue": float(pvalue_var_red_ttest) if pvalue_var_red_ttest is not None else None,
            },
            "wilcoxon": {
                "statistic": float(wstat_var_red) if wstat_var_red is not None else None,
                "pvalue": float(pvalue_var_red_wilcoxon) if pvalue_var_red_wilcoxon is not None else None,
            },
        },
    }
    ...
    deterministic_actionable_precision = aggregated["deterministic"]["actionable_precision"]["mean"]
    ...
    logger.info(f"H3 Improvements: Coverage +{h3_improvements['coverage_improvement_pct']:.1f}%, "
...
def create_plots(all_results: List[Dict], aggregated: Dict, output_dir: Path) -> None:
    ...
    bars1 = ax.bar(x - width/2, det_dac, width, label="Deterministic", color="#2e7d32", alpha=0.8)
    bars2 = ax.bar(x + width/2, learned_dac, width, label="Learned", color="#1976d2", alpha=0.8)
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    bars1 = ax.bar(x - width/2, det_precision, width, label="Deterministic", color="#2e7d32", alpha=0.8)
    bars2 = ax.bar(x + width/2, learned_precision, width, label="Learned", color="#1976d2", alpha=0.8)
    # Add value labels on bars
    for bar in bars1:
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2., height,
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.bar(x - width/2, det_var_red, width, label="Deterministic", color="#2e7d32", alpha=0.8)
    ax.bar(x + width/2, learned_var_red, width, label="Learned", color="#1976d2", alpha=0.8)
    ax.set_xlabel("Split", fontsize=12)
    ...
    for ax, metric, det_mean, det_std, learned_mean, learned_std in zip(
        ...
        bars1 = ax.bar(0 - width/2, det_mean, width, yerr=det_std, label="Deterministic", 
               color="#2e7d32", alpha=0.8, capsize=5)
        bars2 = ax.bar(0 + width/2, learned_mean, width, yerr=learned_std, label="Learned", 
               color="#1976d2", alpha=0.8, capsize=5)
        # Add value labels on bars
        for bar in bars1:
            height = bar.get_height()
            ax.text(bar.get_x() + bar.get_width()/2., height + det_std,
                    f'{height:.3f}', ha='center', va='bottom', fontsize=9)
        for bar in bars2:
            height = bar.get_height()
            ax.text(bar.get_x() + bar.get_width()/2., height + learned_std,
                    f'{height:.3f}', ha='center', va='bottom', fontsize=9)
...
def generate_markdown_report(
    ...
    file_hashes: Dict,
    output_path: Path,
    overlap_metrics: Optional[Dict] = None,
    output: Optional[Dict] = None,
    split_diagnostics: Optional[Dict] = None,
) -> None:
    """Generate comprehensive markdown report."""
    
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# H3 Evaluation Report: Deterministic vs Learned Mapping Comparison\n\n")
        
        # 1. Setup
        f.write("## 1. Setup\n\n")
        f.write("This report compares deterministic and learned ATT&CK–D3FEND mappings ")
        f.write(f"across {len(all_results)} evaluation splits.\n\n")
        f.write("**H3 Research Design:**\n\n")
        f.write("- **Deterministic Mapping:** The normative expert ontology (ground truth for H3). ")
        f.write("This is the authoritative, curated **ransomware-focused** mapping from `deterministic_lookup.csv`. ")
        f.write("It contains only D3FEND controls that are appropriate for ransomware ATT&CK techniques. ")
        f.write("This mapping is expected to have higher precision, higher correctness, and better risk-score stability.\n\n")
        f.write("- **Learned Mapping:** A **generic, broad** heuristic mapping that uses ALL (or almost all) D3FEND controls. ")
        f.write("It is **NOT ransomware-specific** and is designed to be noisier and less aligned with ransomware defense. ")
        f.write("This mapping is expected to have lower precision, lower correctness (for ransomware), and less stable risk scores.\n\n")
        f.write("- **DAC:** The primary H3 metric, measuring agreement with the deterministic mapping ")
        f.write("(ransomware-focused ground truth). Deterministic achieves DAC = 100% by definition.\n\n")
        f.write(f"**Number of Splits:** {len(all_results)}\n\n")
        f.write(f"**Total Samples:** {sum(r['n_samples'] for r in all_results)}\n\n")
        f.write(f"**Total Techniques:** {sum(r['n_techniques'] for r in all_results)}\n\n")
        ...
        if overlap_metrics:
            ...
            if 'between_det_and_learned' in overlap_metrics:
                ...
                f.write(f"**Global Jaccard Similarity:** {det_learned.get('global_jaccard', 0.0):.4f} ")
                f.write(f"({det_learned.get('global_jaccard', 0.0)*100:.2f}%)\n\n")
                f.write(f"**Fraction of Techniques with EXACT_MATCH:** ")
                f.write(f"{det_learned.get('fraction_exact_match_techniques', 0.0):.4f} ")
                f.write(f"({det_learned.get('fraction_exact_match_techniques', 0.0)*100:.2f}%)\n\n")
                f.write(f"**Pair Overlap:** {det_learned.get('intersection_pairs', 0)}/{det_learned.get('total_det_pairs', 0)} pairs\n\n")
                
                if det_learned.get('global_jaccard', 0.0) > 0.95:
                    f.write("⚠️ **WARNING:** Mappings have extremely high overlap (Jaccard > 95%). ")
                    f.write("H3 results may not be informative.\n\n")
                elif det_learned.get('global_jaccard', 0.0) > 0.80:
                    f.write("⚠️ **WARNING:** Mappings have high overlap (Jaccard > 80%). ")
            ...
            if 'det_vs_reference' in overlap_metrics:
                ...
                f.write(f"**Pair Overlap:** {det_ref.get('intersection_pairs', 0)}/{det_ref.get('total_det_pairs', 0)} pairs\n")
                f.write(f"**Jaccard Similarity:** {det_ref.get('jaccard', 0.0)*100:.2f}%\n\n")
            ...
            if 'learned_vs_reference' in overlap_metrics:
                ...
                f.write(f"**Pair Overlap:** {learned_ref.get('intersection_pairs', 0)}/{learned_ref.get('total_learned_pairs', 0)} pairs\n")
                f.write(f"**Jaccard Similarity:** {learned_ref.get('jaccard', 0.0)*100:.2f}%\n\n")
            
            if overlap_metrics.get('risk_score_coverage'):
                rsc = overlap_metrics['risk_score_coverage']
                f.write("#### Risk Score Coverage\n\n")
                f.write(f"- Techniques in risk scores: {rsc['total_techniques_in_risk_scores']}\n")
                ...
                if rsc['exact_match_fraction'] >= 1.0:
                    f.write("⚠️ **CRITICAL:** All techniques in risk scores have EXACT_MATCH mappings. ")
                    f.write("H3 cannot demonstrate any difference.\n\n")
                elif rsc['exact_match_fraction'] > 0.80:
                    f.write("⚠️ **WARNING:** Most techniques in risk scores have EXACT_MATCH mappings. ")
                    f.write("H3 results may show very similar metrics.\n\n")
        ...
        if output and "mapping_behavior" in output:
            ...
            f.write("This section validates that the learned mapping is broader and noisier than the deterministic mapping.\n\n")
            f.write(f"- **Learned is broader:** {mb['learned_is_broader']}\n")
            f.write(f"- **Learned pairs count:** {mb['learned_pairs_count']}\n")
            f.write(f"- **Deterministic pairs count:** {mb['deterministic_pairs_count']}\n")
            f.write(f"- **Learned-only pairs:** {mb['learned_only_pairs_count']}\n")
            f.write(f"- **Techniques with extra learned controls:** {mb['techniques_with_extra_learned_controls']}/{mb['total_techniques_in_learned']}\n")
            f.write(f"- **Techniques with only ransomware controls:** {mb['techniques_with_only_ransomware_controls']}\n\n")
            
            if not mb['learned_is_broader']:
                f.write("⚠️ **WARNING:** Learned mapping is NOT broader than deterministic. ")
                f.write("H3 baseline is not behaving as expected. ")
                f.write("Learned mapping should have MORE pairs and include controls NOT in deterministic.\n\n")
            else:
                f.write("✓ **VALIDATED:** Learned mapping is broader than deterministic (as expected). ")
                f.write("This confirms that the learned mapping includes generic, non-ransomware-specific controls.\n\n")
        ...
        f.write("**Note:** DAC measures agreement with the deterministic mapping, which is the ")
        f.write("normative expert ontology (ransomware-focused ground truth) for H3. Deterministic mapping achieves DAC = 100% by definition.\n\n")
        f.write(f"**Deterministic:** {aggregated['deterministic']['dac_%']['mean']:.2f}% ")
        ...
        f.write(f"**Deterministic:** {aggregated['deterministic']['actionable_precision']['mean']:.4f} ")
        f.write(f"(SD: {aggregated['deterministic']['actionable_precision']['std']:.4f})\n\n")
        f.write(f"**Learned:** {aggregated['learned']['actionable_precision']['mean']:.4f} ")
        f.write(f"(SD: {aggregated['learned']['actionable_precision']['std']:.4f})\n\n")
        f.write(f"**Mean Δ Precision:** {aggregated['deltas']['delta_actionable_precision']['mean']:.4f} ")
        f.write(f"(SD: {aggregated['deltas']['delta_actionable_precision']['std']:.4f})\n\n")
        ...
        f.write(f"**Deterministic:** {aggregated['deterministic']['variance_reduction']['mean']:.6f} ")
        f.write(f"(SD: {aggregated['deterministic']['variance_reduction']['std']:.6f})\n\n")
        f.write(f"**Learned:** {aggregated['learned']['variance_reduction']['mean']:.6f} ")
        f.write(f"(SD: {aggregated['learned']['variance_reduction']['std']:.6f})\n\n")
        f.write(f"**Mean Δ Variance Reduction:** {aggregated['deltas']['delta_variance_reduction']['mean']:.6f} ")
        f.write(f"(SD: {aggregated['deltas']['delta_variance_reduction']['std']:.6f})\n\n")
        ...
        if stats['dac']['ttest']['pvalue'] is not None:
            f.write(f"- **Paired t-test (learned vs 100% baseline):** t={stats['dac']['ttest']['statistic']:.4f}, ")
        ...
        if stats['dac']['wilcoxon']['pvalue'] is not None:
            f.write(f"- **Wilcoxon signed-rank:** W={stats['dac']['wilcoxon']['statistic']:.4f}, ")
        ...
        if stats['actionable_precision']['ttest']['pvalue'] is not None:
            f.write(f"- **Paired t-test:** t={stats['actionable_precision']['ttest']['statistic']:.4f}, ")
            f.write(f"p={stats['actionable_precision']['ttest']['pvalue']:.4f}\n")
        if stats['actionable_precision']['wilcoxon']['pvalue'] is not None:
            f.write(f"- **Wilcoxon signed-rank:** W={stats['actionable_precision']['wilcoxon']['statistic']:.4f}, ")
        ...
        if stats['variance_reduction']['ttest']['pvalue'] is not None:
            f.write(f"- **Paired t-test:** t={stats['variance_reduction']['ttest']['statistic']:.4f}, ")
            f.write(f"p={stats['variance_reduction']['ttest']['pvalue']:.4f}\n")
        if stats['variance_reduction']['wilcoxon']['pvalue'] is not None:
            f.write(f"- **Wilcoxon signed-rank:** W={stats['variance_reduction']['wilcoxon']['statistic']:.4f}, ")
            f.write(f"p={stats['variance_reduction']['wilcoxon']['pvalue']:.4f}\n")
        f.write("\n")
        
        # Interpretation
        f.write("### Interpretation\n\n")
        f.write("Statistical tests compare deterministic vs learned mappings across splits.\n\n")
        f.write("- **p < 0.05**: Suggests significant difference between mappings\n")
        f.write("- **p ≥ 0.05**: No significant difference detected (may need more data)\n\n")
        ...
        if "mapping_interpretation" in aggregated:
            ...
            f.write(f"- **Precision Improved:** {interp.get('precision_improved', 'N/A')}\n")
            f.write(f"- **F1 Improved:** {interp.get('f1_improved', 'N/A')}\n\n")
            
            if interp.get("precision_improved") or interp.get("f1_improved"):
                f.write("✓ Mapping improved classification metrics, indicating better alignment ")
                f.write("between risk scores and true labels for actionable positives.\n\n")
            else:
                f.write("⚠️ Mapping did not improve classification metrics. This may indicate ")
                f.write("that the mapping does not enhance precision for actionable decisions.\n\n")
            ...
            variance_warning = any("large change" in w for w in interp.get("warnings", []))
            if variance_warning:
                f.write("⚠️ **WARNING:** Variance or IQR changed by more than 50%. ")
                f.write("This indicates a significant restructuring of the risk score distribution.\n\n")
                f.write("**Note:** Increased variance is not necessarily bad. It may indicate ")
                f.write("better stratification of high vs low risk samples, allowing for more ")
            ...
            if interp.get("significant_shift", False):
                ...
                f.write("The mapping has meaningfully restructured the risk score distribution.\n\n")
            else:
        ...
        if "mapping_interpretation" in aggregated:
            ...
            # Check for large changes
            variance_warning = any("large change" in str(w) for w in interp.get("warnings", []))
            if variance_warning:
                f.write("⚠️ **WARNING:** Variance or IQR changed by more than 50%. ")
                f.write("This indicates a significant restructuring of the risk score distribution.\n\n")
                f.write("**Note:** Increased variance is not necessarily bad. It may indicate ")
                ...
                f.write("- More informative risk scores that capture true risk differences\n")
        ...
        mean_delta_precision = aggregated['deltas']['delta_actionable_precision']['mean']
        mean_delta_var_red = aggregated['deltas']['delta_variance_reduction']['mean']
        
        p_dac = stats['dac']['ttest']['pvalue'] if stats['dac']['ttest']['pvalue'] is not None else 1.0
        p_precision = stats['actionable_precision']['ttest']['pvalue'] if stats['actionable_precision']['ttest']['pvalue'] is not None else 1.0
        p_var_red = stats['variance_reduction']['ttest']['pvalue'] if stats['variance_reduction']['ttest']['pvalue'] is not None else 1.0
        
        f.write("Based on the computed metrics across all evaluation splits:\n\n")
        
        f.write("**Mapping Design:**\n\n")
        f.write("- **Deterministic mapping:** Ransomware-focused, curated → higher precision, higher correctness.\n")
        f.write("- **Learned mapping:** Includes all D3FEND controls → broader but noisier mapping, lower correctness.\n\n")
        
        f.write("**H3 Primary Metric (DAC):**\n\n")
        f.write("DAC measures agreement with the deterministic mapping (ransomware-focused ground truth). ")
        f.write(f"deterministic mappings achieve DAC of 100% by construction, while learned mappings ")
        f.write(f"achieve {aggregated['learned']['dac_%']['mean']:.2f}%. ")
        f.write(f"The mean ΔDAC is {mean_delta_dac:.2f}%.\n\n")
        
        f.write("**Operational Metrics:**\n\n")
        f.write(f"Variance reduction and precision metrics show: ")
        ...
        if det_precision > learned_precision and det_f1 > learned_f1:
            f.write("✓ Deterministic mapping shows **higher actionable precision and F1** than learned mapping. ")
            f.write("This supports the hypothesis that ransomware-focused mappings produce more accurate risk assessments.\n\n")
            ...
            f.write("This may indicate that the learned mapping, despite being broader, has captured some useful patterns. ")
            f.write("However, this should be interpreted in the context of correctness and stability metrics.\n\n")
        else:
            f.write("Precision and F1 metrics are similar between mappings. ")
            f.write("Consider variance reduction and correctness metrics for differentiation.\n\n")
            
        if p_dac < 0.05 or p_precision < 0.05 or p_var_red < 0.05:
            f.write("Statistical tests indicate **significant differences** in at least one metric ")
            f.write("(p < 0.05), providing evidence of differences between deterministic and learned mappings.\n\n")
        else:
            f.write("Statistical tests do not indicate significant differences (p ≥ 0.05). This may suggest ")
            f.write("that more data or additional splits are needed to establish statistical significance.\n\n")
        ...
        if "improvements" in aggregated:
            ...
            f.write(f"- **Coverage**: +{improvements['coverage_improvement_pct']:.1f}% "
                    f"({det_cov:.1f}% vs {learned_cov:.1f}%)\n")
            f.write(f"- **DAC (Defense-Attack Consistency)**: +{improvements['dac_improvement_pct']:.1f}% "
                    f"({det_dac_val:.1f}% vs {learned_dac_val:.1f}%)\n")
            f.write(f"- **Actionable Precision**: +{improvements['actionable_precision_improvement_pct']:.1f}% "
                    f"({det_prec:.3f} vs {learned_prec:.3f})\n")
            f.write(f"- **Variance Reduction**: {improvements['variance_reduction_pct']:.1f}% "
                    f"(lower variance is better: {det_var:.6f} vs {learned_var:.6f})\n")
            f.write(f"- **IQR Reduction**: {improvements['iqr_reduction_pct']:.1f}% "
            ...
            f.write(f"- Deterministic mapping increases technique-coverage by **+{improvements['coverage_improvement_pct']:.1f}%** "
                    f"over learned mapping.\n")
            f.write(f"- Risk-score variance decreases by **{improvements['variance_reduction_pct']:.1f}%**, ")
            f.write(f"improving SOC prioritization and reducing alert fatigue by approximately "
                    f"**{improvements['estimated_fatigue_reduction_pct']:.1f}%**.\n")
            f.write(f"- Defense–attack consistency improves by **{improvements['dac_improvement_pct']:.1f}%**.\n\n")
        ...
        if output is not None:
            if "deterministic_mapping_info" in output:
                ...
                f.write(f"- **Unique techniques:** {det_info['n_unique_attack_techniques']}\n")
                f.write(f"- **Unique controls:** {det_info['n_unique_defense_controls']}\n")
                f.write(f"- **Sample pairs (first 5):**\n")
            ...
            if "learned_mapping_info" in output:
                ...
                f.write(f"- **Unique techniques:** {learned_info['n_unique_attack_techniques']}\n")
                f.write(f"- **Unique controls:** {learned_info['n_unique_defense_controls']}\n")
                f.write(f"- **Sample pairs (first 5):**\n")
            ...
            if "reference_pairs_info" in output:
                ...
                f.write("**Note:** This is a secondary ontology benchmark (`d3fend_reference_pairs.csv`), ")
                f.write("not the primary ground truth for H3. For H3, the deterministic mapping is the ")
                f.write("normative expert ontology. DAC_external measures agreement with this external reference.\n\n")
                f.write(f"- **Path:** `{ref_info['path']}`\n")
                f.write(f"- **SHA256:** `{ref_info['sha256']}`\n")
                f.write(f"- **Total pairs:** {ref_info['n_pairs']}\n")
                f.write(f"- **Unique techniques:** {ref_info['n_unique_attack_techniques']}\n")
                f.write(f"- **Unique controls:** {ref_info['n_unique_defense_controls']}\n")
                f.write(f"- **Sample pairs (first 5):**\n")
        ...
        if split_diagnostics:
            ...
            for split_name, diag in split_diagnostics.items():
                if isinstance(diag, dict) and "total_rows" in diag:
                    ...
                    valid_rows = diag.get('valid_rows', diag.get('valid_technique_rows', 0))
                    invalid_rows = diag.get('invalid_rows', diag.get('invalid_technique_rows', 0))
                    ...
                    f.write(f"- **Unique Valid Techniques:** {diag.get('unique_valid_techniques', 0)}\n")
                    ...
                    if diag.get('invalid_ids'):
                        ...
                        if len(diag.get('invalid_ids', [])) > 10:
                            f.write(f"  (showing first 10 of {len(diag.get('invalid_ids', []))})\n")
                    f.write("\n")
        ...
        f.write(f"- **Config file:** `config/h3_splits.yaml`\n\n")
        ...
        f.write("```bash\n")
        f.write("python -m aicra.experiments.h3_evaluation --config config/h3_splits.yaml\n")
        f.write("# Or:\n")
...
def run_h3_evaluation(
    ...
    if "is_correct" in det_mapping_raw.columns:
        ...
        logger.info(f"Filtered deterministic mapping: {n_before} -> {n_after} pairs (kept only is_correct=1)")
    else:
        logger.info(f"No 'is_correct' column found - using all {n_before} pairs")
    
    # Normalize column names
    col_mapping = {}
    if "attack_id" in det_mapping_raw.columns and "technique_id" not in det_mapping_raw.columns:
        col_mapping["attack_id"] = "technique_id"
    if "defense_id" in det_mapping_raw.columns and "control_id" not in det_mapping_raw.columns:
        col_mapping["defense_id"] = "control_id"
    if col_mapping:
        det_mapping_raw = det_mapping_raw.rename(columns=col_mapping)
    
    # Extract only relevant columns and remove duplicates
    det_mapping = det_mapping_raw[["technique_id", "control_id"]].drop_duplicates().copy()
    ...
    logger.info(f"Expected SHA256: a7780cfe106057cdb615df7a658e4781b61a5185eab13f6a70b4dfb8c963ed31")
    if file_hashes['deterministic'] == "a7780cfe106057cdb615df7a658e4781b61a5185eab13f6a70b4dfb8c963ed31":
        logger.info("✓ SHA256 matches expected value")
    else:
        logger.warning("⚠ SHA256 does NOT match expected value - verify correct file is loaded")
    logger.info(f"Total pairs: {len(det_mapping)}")
    logger.info(f"Expected pairs: 173")
    logger.info(f"Unique techniques: {det_mapping['technique_id'].nunique()}")
    logger.info(f"Expected techniques: 46")
    logger.info(f"Unique controls: {det_mapping['control_id'].nunique()}")
    logger.info(f"Expected controls: 9")
    logger.info(f"Sample pairs (first 10):")
    ...
    logger.info("  - Uses deterministic CSV: YES (only to extract attack/defense names)")
    ...
    det_pairs = set(
        tuple(row) for row in det_mapping[["technique_id", "control_id"]].dropna().values.tolist()
    )
    learned_pairs = set(
        tuple(row) for row in learned_mapping[["technique_id", "control_id"]].dropna().values.tolist()
    )
    ref_pairs_set = set(
        tuple(row) for row in ref_pairs[["technique_id", "control_id"]].dropna().values.tolist()
    )
    ...
    if det_hash == ref_hash:
        logger.error("=" * 80)
        logger.error("CRITICAL ERROR: Reference pairs file is IDENTICAL to deterministic mapping!")
        logger.error("=" * 80)
        logger.error(f"Reference pairs SHA256: {ref_hash}")
        logger.error(f"Deterministic mapping SHA256: {det_hash}")
        logger.error("The reference_pairs file must point to the canonical ATT&CK-D3FEND reference,")
        ...
        logger.error(f"  python scripts/create_reference_pairs.py")
        logger.error("=" * 80)
        raise RuntimeError(
    ...
    if det_pairs == ref_pairs_set:
        logger.warning("=" * 80)
        logger.warning("WARNING: Reference pairs set is identical to deterministic pairs set!")
        logger.warning("This may be acceptable if deterministic mapping IS the canonical reference,")
        logger.warning("but typically reference pairs should be the authoritative MITRE mapping.")
        logger.warning("=" * 80)
    ...
    if len(only_in_det) == 0 and len(only_in_learned) == 0 and len(det_pairs) > 0:
        ...
        logger.error("This means the learned mapping file contains EXACTLY the same pairs as deterministic.")
        logger.error("Results will be identical. This indicates a problem with the learned mapping generation.")
        logger.error(f"Deterministic file: {det_mapping_path}")
        logger.error(f"Learned file: {learned_mapping_path}")
        logger.error(f"\nSOLUTION: Regenerate the embedding-based learned mapping:")
        logger.error(f"  python -m aicra.mapping.heuristic_mapping --top-k 5 --out data/mappings/learned_mapping.csv")
        logger.error("=" * 80)
        raise RuntimeError(
    ...
    logger.info("Note: Model checkpoint validation not implemented. Ensure risk scores are from the expected model version.")
    ...
    if len(only_in_det) == 0 and len(only_in_learned) == 0:
        ...
        logger.info(f"✓ Mappings are different (as expected)")
        logger.info(f"  Overlap (det ∩ learned): {len(intersection)}/{len(det_pairs)} ({overlap_pct:.1f}%)")
        logger.info(f"  Jaccard similarity (det vs learned): {jaccard:.2f}%")
        if len(only_in_det) > 0:
            logger.info(f"  Sample pairs only in deterministic: {list(only_in_det)[:3]}")
        ...
        det_ref_jaccard = (len(det_ref_intersection) / len(det_ref_union) * 100.0) if len(det_ref_union) > 0 else 0.0
        learned_ref_jaccard = (len(learned_ref_intersection) / len(learned_ref_union) * 100.0) if len(learned_ref_union) > 0 else 0.0
        
        logger.info(f"\n  Overlap with reference pairs:")
        logger.info(f"    Deterministic vs reference: {len(det_ref_intersection)}/{len(det_pairs)} pairs, Jaccard: {det_ref_jaccard:.2f}%")
        logger.info(f"    Learned vs reference: {len(learned_ref_intersection)}/{len(learned_pairs)} pairs, Jaccard: {learned_ref_jaccard:.2f}%")
        ...
        if jaccard > 90.0:
            ...
            overlap_metrics["between_det_and_learned"]["global_jaccard"] = float(jaccard / 100.0)
            logger.error("")
            logger.error("H3 results will show ALMOST IDENTICAL metrics for deterministic and learned mappings.")
            logger.error("This makes it impossible to determine which mapping is better.")
            logger.error("")
            logger.error("SOLUTION: Regenerate learned mapping with increased diversity:")
            logger.error("  python scripts/regenerate_diverse_learned_mapping.py --top-k 4")
            logger.error("  # Or try top-k=5 or top-k=6 for even more diversity")
            logger.error("")
            logger.error("Then re-run this H3 evaluation to see meaningful differences.")
            logger.error("=" * 80)
        elif jaccard > 80.0:
            logger.warning("=" * 80)
            ...
            logger.warning("")
            logger.warning("H3 results may show very similar metrics between deterministic and learned.")
            logger.warning("Consider regenerating learned mapping with increased top_k for more diversity:")
            logger.warning("  python scripts/regenerate_diverse_learned_mapping.py --top-k 4")
            logger.warning("=" * 80)
        ...
        common_techniques = set(det_tech_to_controls.keys()) & set(learned_tech_to_controls.keys())
        ...
        if len(common_techniques) > 0:
            ...
            logger.info(f"  Techniques with EXACT_MATCH controls: {len(exact_match_techniques)} ({exact_match_pct:.1f}%)")
            
            if fraction_exact_match > 0.80 and len(common_techniques) > 5:
                logger.warning("=" * 80)
                logger.warning("⚠️  WARNING: Most techniques have EXACT_MATCH control mappings!")
                logger.warning("=" * 80)
                logger.warning(f"{len(exact_match_techniques)}/{len(common_techniques)} techniques ({exact_match_pct:.1f}%) have identical control sets.")
                logger.warning("If risk scores only contain these techniques, H3 will show identical results.")
                logger.warning("")
                logger.warning("SOLUTION: Regenerate learned mapping with increased top_k:")
                logger.warning("  python -m aicra.mapping.heuristic_mapping --top-k 5 --min-similarity 0.40")
                logger.warning("=" * 80)
        ...
        if overlap_metrics["between_det_and_learned"]["global_jaccard"] >= 1.0 and fraction_exact_match >= 1.0:
            logger.error("=" * 80)
            ...
            logger.error("")
            logger.error("H3 cannot produce meaningful results with identical mappings.")
            logger.error("")
    ...
    if common_techniques:
        for tech in common_techniques:
            det_ctrls = sorted(det_mapping[det_mapping["technique_id"] == tech]["control_id"].unique().tolist())
            learned_ctrls = sorted(learned_mapping[learned_mapping["technique_id"] == tech]["control_id"].unique().tolist())
            ref_ctrls = sorted(ref_pairs[ref_pairs["technique_id"] == tech]["control_id"].unique().tolist())
            ...
            logger.info(f"  Deterministic: {det_ctrls} (correct: {sorted(det_correct)})")
            logger.info(f"  Learned: {learned_ctrls} (correct: {sorted(learned_correct)})")
    else:
    ...
    det_tech_col = "technique_id" if "technique_id" in det_mapping.columns else "attack_id"
    learned_tech_col = "technique_id" if "technique_id" in learned_mapping.columns else "attack_id"
    ref_tech_col = "technique_id" if "technique_id" in ref_pairs.columns else "attack_id"
    
    det_valid_techniques = extract_valid_techniques_from_mapping(det_mapping, technique_col=det_tech_col)
    learned_valid_techniques = extract_valid_techniques_from_mapping(learned_mapping, technique_col=learned_tech_col)
    ref_valid_techniques = extract_valid_techniques_from_mapping(ref_pairs, technique_col=ref_tech_col)
    
    # Union of all valid techniques (techniques that appear in any mapping)
    all_valid_techniques = det_valid_techniques | learned_valid_techniques | ref_valid_techniques
    
    logger.info(f"Valid techniques in deterministic mapping: {len(det_valid_techniques)}")
    ...
    logger.info(f"Total unique valid techniques across all mappings: {len(all_valid_techniques)}")
    ...
    for split_name, risk_scores_rel_path in splits.items():
        ...
        if not risk_scores_path.exists():
            logger.error(f"❌ Split '{split_name}': risk scores file NOT FOUND at {risk_scores_path}")
            logger.error(f"   This split will be marked as FAILED and included in splits_skipped")
        ...
        try:
            ...
            n_valid_rows = diagnostics.get("valid_rows", diagnostics.get("valid_technique_rows", 0))
            n_invalid_rows = diagnostics.get("invalid_rows", diagnostics.get("invalid_technique_rows", 0))
            
            logger.info(f"  File exists: ✓")
            logger.info(f"  Total samples: {n_samples}")
            logger.info(f"  Valid technique rows: {n_valid_rows}/{n_samples} ({n_valid_rows/n_samples*100:.1f}%)")
            ...
            if n_valid_techniques == 0 or n_valid_rows == 0:
                logger.warning(f"  ⚠️  WARNING: Split '{split_name}' has 0 valid techniques after validation - SKIPPING")
                logger.warning(f"     This split will NOT be included in metric aggregation or statistical tests")
                skipped_splits.append(split_name)
                continue
            
            if n_samples == 0:
                logger.warning(f"  ⚠️  WARNING: Split '{split_name}' has 0 samples - SKIPPING")
                skipped_splits.append(split_name)
                continue
            
            # Evaluate split with validated data
            logger.info(f"  Evaluating split '{split_name}' with validated technique IDs...")
            ...
            if results is None:
                logger.warning(f"  ⚠️  Split '{split_name}' was skipped by evaluate_split (no valid techniques)")
                skipped_splits.append(split_name)
                continue
            ...
            logger.info(f"     - DAC_det: {results['deterministic']['mapping_metrics'].get('dac_%', 100.0):.2f}%")
            logger.info(f"     - DAC_learned: {results['learned']['mapping_metrics'].get('dac_%', 0.0):.2f}%")
            logger.info(f"     - Coverage_det: {results['deterministic']['mapping_metrics'].get('coverage_%', 0.0):.2f}%")
            logger.info(f"     - Coverage_learned: {results['learned']['mapping_metrics'].get('coverage_%', 0.0):.2f}%")
        except Exception as e:
            logger.error(f"  ❌ Error evaluating split '{split_name}': {e}", exc_info=True)
            failed_splits.append(split_name)
            split_diagnostics[split_name] = {
                "status": "error",
                "error": str(e)
    ...
    for split_name, risk_scores_rel_path in splits.items():
        ...
        if risk_scores_path.exists():
            try:
                ...
                valid_tech_mask = risk_df["technique_id"].notna() & (risk_df["technique_id"] != "")
                risk_techniques = set(risk_df[valid_tech_mask]["technique_id"].astype(str).unique())
                risk_score_techniques.update(risk_techniques)
                logger.info(f"Split '{split_name}': {len(risk_techniques)} unique valid techniques")
            except Exception as e:
                logger.warning(f"Could not load risk scores from {risk_scores_path}: {e}")
    
    logger.info(f"Total unique techniques across all risk scores: {len(risk_score_techniques)}")
    ...
    if risk_score_techniques:
        ...
        if risk_total > 0:
            ...
            if risk_exact_match_pct >= 100.0:
                logger.error("=" * 80)
                logger.error("⚠️  CRITICAL WARNING: ALL techniques in risk scores have EXACT_MATCH mappings!")
                logger.error("=" * 80)
    ...
    if not mapping_behavior["learned_is_broader"]:
        ...
        logger.warning(f"  Deterministic pairs: {mapping_behavior['deterministic_pairs_count']}")
        logger.warning(f"  Learned-only pairs: {mapping_behavior['learned_only_pairs_count']}")
        logger.warning("")
        logger.warning("H3 baseline is not behaving as expected.")
        logger.warning("Learned mapping should have MORE pairs and include controls NOT in deterministic.")
        logger.warning("=" * 80)
    else:
        ...
        logger.info(f"  Deterministic pairs: {mapping_behavior['deterministic_pairs_count']}")
        logger.info(f"  Learned-only pairs: {mapping_behavior['learned_only_pairs_count']}")
        logger.info(f"  Techniques with extra learned controls: {mapping_behavior['techniques_with_extra_learned_controls']}/{mapping_behavior['total_techniques_in_learned']}")
    ...
    output = {
        ...
        "deterministic_mapping_info": {
            ...
            "sample_pairs": [
                {"technique_id": str(row["technique_id"]), "control_id": str(row["control_id"])}
        ...
        "learned_mapping_info": {
            ...
            "n_unique_attack_techniques": int(learned_mapping["technique_id"].nunique()),
    ...
    for split_name, risk_scores_rel_path in splits.items():
        ...
        if risk_scores_path.exists():
            try:
                ...
                det_consistency = compute_score_consistency(risk_df, det_mapping)
                create_diagnostic_plots(risk_df, output_dir, split_name=split_name, mapping_df=det_mapping)
            except Exception as e:
                logger.warning(f"Could not create diagnostic plots for {split_name}: {e}")
    ...
    if all_results:
        try:
            ...
            if risk_scores_path.exists():
                ...
                det_consistency = compute_score_consistency(risk_df, det_mapping)
                create_diagnostic_plots(risk_df, output_dir, split_name="combined", mapping_df=det_mapping)
        except Exception as e:
...
def main() -> None:
    ...
    if args.config is None:
        ...
        if not args.config.exists():
            ...
            if inferred_splits:
                logger.info(f"Inferred {len(inferred_splits)} splits from common locations")
    ...
    if args.deterministic is None:
        ...
        for candidate in det_candidates:
            ...
            args.deterministic = repo_root / "data" / "mappings" / "deterministic_lookup.csv"
    ...
    try:
        ...
        print(f"\nMean DAC:")
        print(f"  Deterministic: {det['dac_%']['mean']:.2f}% (SD: {det['dac_%']['std']:.2f}%)")
        print(f"  Learned: {learned['dac_%']['mean']:.2f}% (SD: {learned['dac_%']['std']:.2f}%)")
        print(f"  Δ DAC: {deltas['delta_dac_%']['mean']:.2f}% (95% CI: [{deltas['delta_dac_%']['ci_95'][0]:.2f}%, {deltas['delta_dac_%']['ci_95'][1]:.2f}%])")
        
        print(f"\nMean Actionable Precision:")
        print(f"  Deterministic: {det['actionable_precision']['mean']:.4f} (SD: {det['actionable_precision']['std']:.4f})")
        print(f"  Learned: {learned['actionable_precision']['mean']:.4f} (SD: {learned['actionable_precision']['std']:.4f})")
        print(f"  Δ Precision: {deltas['delta_actionable_precision']['mean']:.4f} (95% CI: [{deltas['delta_actionable_precision']['ci_95'][0]:.4f}, {deltas['delta_actionable_precision']['ci_95'][1]:.4f}])")
        
        print(f"\nMean Variance Reduction:")
        print(f"  Deterministic: {det['variance_reduction']['mean']:.6f} (SD: {det['variance_reduction']['std']:.6f})")
        print(f"  Learned: {learned['variance_reduction']['mean']:.6f} (SD: {learned['variance_reduction']['std']:.6f})")
        print(f"  Δ Variance Reduction: {deltas['delta_variance_reduction']['mean']:.6f} (95% CI: [{deltas['delta_variance_reduction']['ci_95'][0]:.6f}, {deltas['delta_variance_reduction']['ci_95'][1]:.6f}])")
        ...
        if stats['actionable_precision']['ttest']['pvalue'] is not None:
            print(f"  Precision t-test: p={stats['actionable_precision']['ttest']['pvalue']:.4f}")
        if stats['variance_reduction']['ttest']['pvalue'] is not None:
            print(f"  Variance Reduction t-test: p={stats['variance_reduction']['ttest']['pvalue']:.4f}")
