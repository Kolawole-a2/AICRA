...
from ..core.evaluation import Metrics, evaluate_probs, cost_sensitive_threshold
...
def run_h1_classification_experiment(
    ...
    try:
        ...
        logger.info(f"Train samples: {len(train_data.features)}, Test samples: {len(test_data.features)}")
        ...
        if train_data.timestamps is not None and test_data.timestamps is not None:
            ...
            if train_max_ts >= test_min_ts:
                logger.warning(f"Time split integrity issue: train max timestamp ({train_max_ts}) >= test min timestamp ({test_min_ts})")
            else:
                logger.info(f"Time-ordered split verified: train max={train_max_ts}, test min={test_min_ts}")
    ...
    # Prepare features
    if use_pe_features and hasattr(train_data, 'file_paths') and train_data.file_paths is not None:
    ...
    logger.info(f"Baseline metrics: AUROC={best_baseline.auroc:.4f}, "
    ...
    logger.info(f"Banking-optimized threshold: {banking_threshold:.4f} (FN cost={banking_cost_fn}, FP cost={banking_cost_fp})")
    ...
    metrics = {
        ...
        "operational_threshold_legacy": float(operational_threshold),  # Keep for backward compatibility
        "precision": aicra_precision,
        "recall": aicra_recall,
        "f1": aicra_f1,
        ...
        "baseline": {
            "logistic_regression": {
                ...
                "false_negatives": baseline_results['logistic_regression'].false_negatives,
            },
            "majority_classifier": {
                ...
                "false_negatives": baseline_results['majority_classifier'].false_negatives,
            },
            "best_baseline": {
        ...
        "alert_fatigue_reduction": {
            ...
            "fn_reduction_absolute": int(best_baseline.false_negatives - fn) if best_baseline.false_negatives is not None else 0,
        ...
        # ====================================================================
        "improvement_statement": format_improvement_statement('H1', {
            'auroc_pct': improvements.auroc_pct,
    ...
    if hasattr(train_data, 'families') and train_data.families is not None and \
       hasattr(test_data, 'families') and test_data.families is not None:
        
        train_families = train_data.families.values if hasattr(train_data.families, 'values') else train_data.families
        test_families = test_data.families.values if hasattr(test_data.families, 'values') else test_data.families
        ...
        if len(held_out_families) > 0:
            logger.info(f"Out-of-family test: {len(held_out_families)} held-out families found")
            logger.info(f"Held-out families: {list(held_out_families)[:10]}...")  # Log first 10
            ...
            if oof_mask.sum() > 10:  # Only evaluate if enough samples
                ...
                logger.info(f"Out-of-family metrics: AUROC={oof_auroc:.4f}, PR-AUC={oof_pr_auc:.4f}, n_samples={oof_mask.sum()}")
            else:
                logger.warning(f"Out-of-family test skipped: only {oof_mask.sum()} samples in held-out families")
        else:
        ...
        if len(all_families) > 0:
            logger.info(f"Evaluating per-family metrics across {len(all_families)} families...")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write("AUROC >= 0.95 and operational precision suitable for banking environments.\n\n")
        f.write("## Metrics\n\n")
        ...
        f.write(f"- **Operational Threshold (Banking-optimized)**: {banking_thresh:.4f}\n")
        f.write(f"- **Cost Parameters**: FN={metrics.get('cost_parameters', {}).get('cost_fn', 'N/A')}, FP={metrics.get('cost_parameters', {}).get('cost_fp', 'N/A')}\n")
        f.write(f"- **Precision** (threshold={banking_thresh:.4f}): {metrics['precision']:.4f}\n")
        f.write(f"- **Recall** (threshold={banking_thresh:.4f}): {metrics['recall']:.4f}\n")
        f.write(f"- **F1** (threshold={banking_thresh:.4f}): {metrics['f1']:.4f}\n")
        f.write(f"- **Confusion Matrix**: TN={metrics.get('confusion_matrix', {}).get('tn', 'N/A')}, FP={metrics.get('confusion_matrix', {}).get('fp', 'N/A')}, FN={metrics.get('confusion_matrix', {}).get('fn', 'N/A')}, TP={metrics.get('confusion_matrix', {}).get('tp', 'N/A')}\n")
        ...
        if "oof_auroc" in metrics:
            f.write(f"- **Out-of-Family AUROC** (held-out families): {metrics['oof_auroc']:.4f}\n")
            f.write(f"- **Out-of-Family PR-AUC**: {metrics.get('oof_pr_auc', 'N/A')}\n")
            f.write(f"- **Out-of-Family Samples**: {metrics.get('oof_n_samples', 'N/A')}\n")
            f.write(f"- **Held-out Families**: {metrics.get('oof_n_families', 'N/A')}\n")
        elif "per_family_auroc_mean" in metrics:
            f.write(f"- **Per-Family AUROC**: {metrics['per_family_auroc_mean']:.4f} ± {metrics['per_family_auroc_std']:.4f}\n")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run H1 classification experiment"
    )
    parser.add_argument(
        "--output",
        type=Path,
