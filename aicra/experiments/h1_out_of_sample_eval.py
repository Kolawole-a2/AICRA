...
def evaluate_temporal_holdout(
    model_path: Path,
    train_time_end: Optional[pd.Timestamp],
    test_time_start: Optional[pd.Timestamp],
    output_dir: Path
) -> Dict:
    """
...
def evaluate_out_of_family_temporal(
    ...
    temporal_mask = test_data.timestamps >= test_time_start if test_time_start else pd.Series([True] * len(test_data.timestamps))
...
def main():
    ...
    parser.add_argument("--train-families", nargs="+", help="List of families seen during training (for OOF eval)")
    ...
    if args.train_families:
        ...
        logger.warning("--train-families not provided, skipping OOF+temporal evaluation")
