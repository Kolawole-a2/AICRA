...
def run_h2_calibration_thresholds_experiment(
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        cost_better = (
            metrics["cost_optimized"]["calibrated"]["expected_loss"]
            < metrics["f1_optimized"]["calibrated"]["expected_loss"]
        )
        if cal_improved and cost_better:
            f.write(
                "✓ H2 is **supported**: Calibration improves Brier score and cost-optimal "
            )
            f.write(
                "threshold reduces expected loss compared to F1-optimized threshold.\n\n"
            )
