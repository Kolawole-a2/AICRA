...
def load_attack_techniques_with_descriptions(
    ...
    if csv_path and csv_path.exists():
        ...
        for col in df.columns:
            col_lower = col.lower()
            if "technique_id" in col_lower or (
                "attack_id" in col_lower and "technique" in col_lower
            ):
        ...
        if "description" in df.columns and df["description"].notna().any():
            ...
            if all(col in df.columns for col in required_cols):
                ...
                LOGGER.info(
                    f"Loaded {len(df)} ATT&CK techniques from CSV with descriptions"
                )
                return df
    ...
    if stix_path and stix_path.exists():
        ...
        attack_patterns = memory_store.query(
        ...
        df = df.dropna(subset=["technique_id"])
        LOGGER.info(
            f"Extracted {len(df)} ATT&CK techniques from STIX with descriptions"
        )
        return df
...
def load_d3fend_controls_with_descriptions(
    ...
    if csv_path and csv_path.exists():
        ...
        for col in df.columns:
            ...
            elif "name" in col_lower or (
                "technique" in col_lower and "d3fend" in col_lower
            ):
                col_mapping[col] = "name"
...
def build_heuristic_mapping(
...
) -> tuple[pd.DataFrame, pd.DataFrame | None, pd.DataFrame | None, np.ndarray | None]:
    """
    ...
    LOGGER.info(
        f"Building heuristic mapping with config: top_k={config.top_k}, "
    ...
    LOGGER.info(
        f"Using {len(attack_df)} attacks and {len(defense_df)} controls with descriptions"
    )
    ...
    if SENTENCE_TRANSFORMERS_AVAILABLE:
        ...
        attack_embeddings = model.encode(
            attack_texts, show_progress_bar=True, batch_size=32
        )

        LOGGER.info("Computing embeddings for controls...")
        control_embeddings = model.encode(
            control_texts, show_progress_bar=True, batch_size=32
        )
    ...
    if len(result_df) == 0:
        ...
        LOGGER.info(
            f"Average similarity score: {result_df['similarity_score'].mean():.4f}"
        )
        ...
        LOGGER.info(
            f"Mappings per technique: {len(result_df) / len(attack_df):.2f} (avg)"
        )
    ...
    if check_diversity:
        try:
            ...
            LOGGER.error(
                "Returning mapping anyway, but it may be identical to deterministic"
            )
    ...
    try:
        ...
        LOGGER.error("=" * 80)
        LOGGER.error(
            "SANITY CHECK FAILED: Learned mapping is not broader than deterministic!"
        )
        LOGGER.error("=" * 80)
        raise
...
def validate_learned_is_broader(learned_mapping_df: pd.DataFrame) -> None:
    ...
    if deterministic_path is None or not deterministic_path.exists():
        LOGGER.warning(
            "Deterministic mapping not found, skipping broader-than-deterministic check"
        )
        return

    LOGGER.info("=" * 80)
    LOGGER.info(
        "SANITY CHECK: Validating learned mapping is broader than deterministic"
    )
    LOGGER.info("=" * 80)
    ...
    det_pairs = set(
        zip(
            det_df[det_tech_col].dropna().astype(str),
            det_df[det_ctrl_col].dropna().astype(str),
            strict=False,
        )
    )
    learned_pairs = set(
        zip(
            learned_mapping_df["technique_id"].dropna().astype(str),
    ...
    if techniques_with_only_det_controls:
        LOGGER.warning(
            f"  Techniques with only deterministic controls: {len(techniques_with_only_det_controls)}"
        )
    LOGGER.info("=" * 80)
...
def ensure_diversity_from_deterministic(
    ...
    attack_df: pd.DataFrame | None = None,
    defense_df: pd.DataFrame | None = None,
    similarity_matrix: np.ndarray | None = None,
    config: HeuristicMappingConfig | None = None,
) -> pd.DataFrame:
    """
    ...
    if det_pairs == learned_pairs and len(det_pairs) > 0:
        ...
        if (
            config
            and attack_df is not None
            and defense_df is not None
            ...
            adjusted_pairs = set(
                zip(
                    adjusted_df["technique_id"].dropna().astype(str),
        ...
        if (
            ...
            if adjusted_pairs != det_pairs:
                LOGGER.info(
                    "✓ Diversity adjustment successful by prioritizing non-deterministic controls"
                )
                return adjusted_df
            else:
                LOGGER.error("=" * 80)
                LOGGER.error(
                    "CRITICAL: Learned mapping is still identical after diversity adjustment!"
                )
                LOGGER.error("=" * 80)
                raise RuntimeError(
