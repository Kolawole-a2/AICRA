...
def perform_statistical_tests(
    ...
    if len(results_df) > 1:
        ...
        t_stat_precision, p_value_precision = ttest_rel(precision_det, precision_learned)
        ...
        try:
            w_stat, w_pvalue = wilcoxon(precision_det, precision_learned, alternative="greater")
    ...
    if len(dac_values_var) > 2:
        ...
        stats_results["dac_vs_variance_reduction_correlation"] = {"error": "Insufficient data"}
    ...
    for test_name, test_result in stats_results.items():
        if "error" not in test_result:
            summary_rows.append({
                "test": test_name,
                "statistic": test_result.get("statistic") or test_result.get("correlation"),
...
def create_correlation_plots(
    ...
    plt.scatter(dac_det, precision_det, label="Deterministic", color="#2e7d32", s=100, alpha=0.7)
    plt.scatter(dac_learned, precision_learned, label="Learned", color="#1976d2", s=100, alpha=0.7)
    plt.xlabel("DAC (Defense-Attack Consistency)", fontsize=12)
    plt.ylabel("Precision", fontsize=12)
    plt.title("DAC vs Precision: Higher DAC Predicts Better Precision", fontsize=14, fontweight='bold')
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(dac_det, var_red_det, label="Deterministic", color="#2e7d32", s=100, alpha=0.7)
    plt.scatter(dac_learned, var_red_learned, label="Learned", color="#1976d2", s=100, alpha=0.7)
    plt.xlabel("DAC (Defense-Attack Consistency)", fontsize=12)
    plt.ylabel("Variance Reduction", fontsize=12)
    plt.title("DAC vs Variance Reduction: Higher DAC Predicts Better Stability", fontsize=14, fontweight='bold')
    plt.legend(fontsize=11)
    plt.grid(alpha=0.3)
