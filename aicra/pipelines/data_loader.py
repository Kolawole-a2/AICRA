...
class EMBERDataLoader:
    ...
    def load_ember_data(
        ...
        print(
            f"Loading EMBER-2024 data from {len(jsonl_files)} JSONL files in {data_path}"
        )
                    ...
                    for line_num, line in enumerate(f, 1):
                        try:
                            ...
                            if file_idx <= 5:  # Only show errors for first few files
                                print(
                                    f"  WARNING: Skipping invalid JSON on line {line_num} in {jsonl_file.name}: {e}"
                                )
                            continue
        ...
        features_df, labels_series, families_series = (
            self._extract_features_labels_families(df)
        )
        ...
        if sample_size is not None and len(features_df) > sample_size:
            ...
            sample_indices = np.random.choice(
                len(features_df), size=sample_size, replace=False
            )
            ...
            families_series = families_series.iloc[sample_indices].reset_index(
                drop=True
            )
    ...
    def _validate_schema(self, df: pd.DataFrame, phase: str) -> None:
        ...
        feature_cols = [
            col for col in df.columns if col.startswith(("feature_", "byte_", "pe_"))
        ]
        if not feature_cols:
            raise RuntimeError(
        ...
        print(
            f"✅ Schema validation passed: {len(feature_cols)} features, label column: {label_col}"
        )

    def _extract_features_labels_families(
    ...
    def _save_data_summary(self, metadata: dict[str, Any], phase: str) -> None:
        ...
        print(
            f"  ⚖️  Prevalence: {metadata['prevalence']:.3f} ({metadata['positive_samples']:,} positive, {metadata['negative_samples']:,} negative)"
        )
