...
class SmokeTestPipeline:
    ...
    def _create_synthetic_sample(self, output_path: Path) -> None:
        ...
        families = np.random.choice(
            ["lockbit", "conti", "ryuk", "unknown"], n_samples, p=[0.3, 0.3, 0.2, 0.2]
        )
    ...
    def _detect_dataset(self) -> tuple[str, str]:
        ...
        if ember_path.exists():
            ...
            raise FileNotFoundError(
                "No dataset found. Expected EMBER-2024.csv or sample.csv"
            )
        ...
        try:
            ...
            train_data = Dataset(
                ...
                timestamps=pd.Series(
                    pd.date_range("2024-01-01", periods=len(labels), freq="H")
                ),
            )
    ...
    def _run_evaluation(self, model_path: str) -> Any:
        ...
        # Basic metrics
        auroc = roc_auc_score(
            test_data.labels.values, y_prob[:, 1]
        ...
        ece = self._compute_simple_ece(
            test_data.labels.values, y_prob[:, 1], n_bins=3
        )  # Reduced bins
        ...
        k_samples = max(
            1, int(len(test_data.labels.values) * 5 / 100)
    ...
    def _create_mock_artifacts(self, metrics) -> None:
        ...
        for plot_file in plot_files:
            ...
            smoke_plot_path = self.artifacts_dir / plot_file.replace(
                ".png", "_smoke.png"
            )
            smoke_plot_path.write_bytes(b"mock plot data")

    def _compute_simple_ece(
        self, y_true: np.ndarray, y_prob: np.ndarray, n_bins: int = 5
    ) -> float:
    ...
    def _run_calibration(self, model_path: str) -> str:
        ...
        try:
            ...
            train_data = Dataset(
                ...
                timestamps=pd.Series(
                    pd.date_range("2024-01-01", periods=len(train_df), freq="H")
                ),
            )
            ...
            val_data = Dataset(
                ...
                timestamps=pd.Series(
                    pd.date_range("2024-01-01", periods=len(val_df), freq="H")
                ),
            )
    ...
    def _run_thresholds(self, model_path: str, calibrator_path: str) -> float:
        ...
        try:
            ...
            test_data = Dataset(
                ...
                timestamps=pd.Series(
                    pd.date_range("2024-01-01", periods=len(df), freq="H")
                ),
            )
    ...
    def _run_register(self, model_path: str, policy_path: str) -> str:
        ...
        register_df["attack_techniques"] = mapping_results.apply(
            lambda x: str(x[1])
        )  # Convert to string
        register_df["d3fend_controls"] = mapping_results.apply(
            lambda x: str(x[2])
        )  # Convert to string
        ...
        register_df["expected_loss"] = register_df["susceptibility"] * float(
            policy.impact_default
        )
        ...
        register_df["prescriptive_controls"] = register_df[
            "susceptibility_bucket"
        ].apply(lambda x: str(get_controls(x)))
        register_df["status"] = np.where(
            register_df["susceptibility"] >= policy.threshold, "Alert", "Monitor"
        )
    ...
    def _validate_artifacts_and_metrics(self, metrics: Any) -> dict[str, Any]:
        ...
        if not register_path.exists():
            ...
            try:
                ...
                missing_cols = [
                    col for col in required_cols if col not in register_df.columns
                ]
                if missing_cols:
