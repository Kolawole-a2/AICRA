    ...
    def _extract_pe_headers(self, pe_data: bytes) -> dict[str, Any]:
        ...
        try:
            ...
            machine = struct.unpack(
                "<H", pe_data[coff_header_offset : coff_header_offset + 2]
            )[0]
            num_sections = struct.unpack(
                "<H", pe_data[coff_header_offset + 2 : coff_header_offset + 4]
            )[0]
            timestamp = struct.unpack(
                "<I", pe_data[coff_header_offset + 4 : coff_header_offset + 8]
            )[0]
            entry_point = struct.unpack(
                "<I", pe_data[coff_header_offset + 16 : coff_header_offset + 20]
            )[0]
            ...
            magic = struct.unpack(
                "<H", pe_data[opt_header_offset : opt_header_offset + 2]
            )[0]
            ...
            for _i in range(min(num_sections, 16)):  # Limit to 16 sections for safety
                ...
                characteristics = struct.unpack(
                    "<I", pe_data[section_offset + 36 : section_offset + 40]
                )[0]
                section_flags.append(characteristics)

                # Virtual size
                virtual_size = struct.unpack(
                    "<I", pe_data[section_offset + 8 : section_offset + 12]
                )[0]
    ...
    def _extract_entropy_stats(self, pe_data: bytes) -> dict[str, float]:
        ...
        try:
            ...
            if len(pe_data) > 64 and pe_data[:2] == b"MZ":
                pe_offset = struct.unpack("<I", pe_data[60:64])[0]
                if (
                    pe_offset < len(pe_data) - 4
                    and pe_data[pe_offset : pe_offset + 4] == b"PE\x00\x00"
                ):
                    section_entropies = self._extract_section_entropies(
                        pe_data, pe_offset
                    )
                    ...
                    if section_entropies:
                        features.update(
                            {
                                "entropy_section_mean": float(
                                    np.mean(section_entropies)
                                ),
                                "entropy_section_median": float(
                                    np.median(section_entropies)
                                ),
    ...
    def _extract_section_entropies(self, pe_data: bytes, pe_offset: int) -> list[float]:
        ...
        try:
            ...
            for _i in range(min(num_sections, 16)):  # Limit for safety
                ...
                raw_data_offset = struct.unpack(
                    "<I", pe_data[section_offset + 20 : section_offset + 24]
                )[0]
                raw_data_size = struct.unpack(
                    "<I", pe_data[section_offset + 16 : section_offset + 20]
                )[0]

                if (
                    raw_data_offset > 0
                    and raw_data_size > 0
                    and raw_data_offset + raw_data_size <= len(pe_data)
                ):
                    section_data = pe_data[
                        raw_data_offset : raw_data_offset + raw_data_size
                    ]
...
def build_pe_features(
    dataframe: pd.DataFrame, file_path_column: str = "file_path"
) -> pd.DataFrame:
    ...
    expected_columns.update(
        [
            ...
            "pe_section_flags_mean",
            "entropy_overall",
            "entropy_section_mean",
            "entropy_section_median",
            "entropy_section_max",
            "entropy_section_std",
            "entropy_section_count",
        ]
    )
