...
class MappingPipeline:
    ...
    def _load_canonical_families(self) -> dict[str, str]:
        ...
        if not self.skip_mlflow:
            mlflow.log_param(
                "canonical_families_version", data.get("__version__", "unknown")
            )
    ...
    def _load_family_to_attack(self) -> dict[str, list[str]]:
        ...
        if not self.skip_mlflow:
            mlflow.log_param(
                "family_to_attack_version", data.get("__version__", "unknown")
            )
    ...
    def _load_attack_to_d3fend(self) -> dict[str, list[str]]:
        ...
        with open(d3fend_path, encoding="utf-8") as f:
            data = yaml.safe_load(f)

        # Log version to MLflow
        if not self.skip_mlflow:
            mlflow.log_param(
                "attack_to_d3fend_version", data.get("__version__", "unknown")
            )
    ...
    def map_families_batch(
        self, raw_families: list[str], phase: str = "unknown"
    ) -> pd.DataFrame:
        """
        ...
        df["canonical_family"] = (
            df["normalized_family"].map(self.canonical_families).fillna("Unknown")
        )

        # Step 3: Map canonical families to techniques
        df["techniques"] = (
            df["canonical_family"]
            .map(self.family_to_attack_dict)
            .fillna("")
        ...
        techniques_df["d3fend_controls"] = (
            techniques_df["techniques"]
            .map(self.attack_to_d3fend_dict)
            .fillna("")
            .apply(lambda x: x if isinstance(x, list) else [])
        )
        ...
        result_df = (
            techniques_df.groupby("raw_family")
            .agg(
                {
        ...
        if not self.skip_mlflow:
            ...
            mlflow.log_metric(
                f"mapping_samples_per_second_{phase}", len(raw_families) / elapsed_time
            )

        return result_df

    def _update_coverage_stats(
        self, input_df: pd.DataFrame, result_df: pd.DataFrame, phase: str
    ):
        """Update coverage statistics."""
        # Alias to family coverage
        total_samples = len(input_df)
        mapped_families = len(result_df[result_df["canonical_family"] != "Unknown"])
        unmapped_aliases = input_df[
            ~input_df["normalized_family"].isin(self.canonical_families.keys())
        ]["raw_family"].tolist()

        # Filter out NaN values
        unmapped_aliases = [
            alias for alias in unmapped_aliases if pd.notna(alias) and alias != ""
        ]
        ...
        mapped_families_with_techniques = len(
            result_df[result_df["techniques"].apply(len) > 0]
        )
        unmapped_families = result_df[result_df["techniques"].apply(len) == 0][
            "canonical_family"
        ].tolist()

        # Filter out NaN values
        unmapped_families = [
            family for family in unmapped_families if pd.notna(family) and family != ""
        ]
        ...
        all_techniques = [
            t
            for techniques in result_df["techniques"]
            for t in techniques
            if pd.notna(t) and t != ""
        ]
        mapped_techniques = len(
            [t for t in all_techniques if t in self.attack_to_d3fend_dict]
        )
        unmapped_techniques = [
            t for t in all_techniques if t not in self.attack_to_d3fend_dict
        ]
        ...
        for stage, stats in self.coverage_stats.items():
            ...
            for item, count in unmapped_counts.items():
                unmapped_data.append(
                    {"stage": stage, "unmapped_item": item, "count": count}
                )
    ...
    def check_coverage_thresholds(self, phase: str) -> bool:
        ...
        if (
            self.settings.dataset_type == "ember"
            and not self.settings.require_family_mapping
        ):
            if alias_coverage < (1 - max_unmapped_rate):
                print(
                    f"⚠️  WARN: Low alias-to-family coverage ({alias_coverage:.3f}) for EMBER dataset"
                )
                print(
                    "   This is expected for EMBER datasets. Proceeding with reduced coverage."
                )
