...
class FullDebugPipeline:
    ...
    def validate_data_loading(
        ...
        if len(y_unique) < 2:
            raise RuntimeError(
                "CRITICAL: Only one class found in labels. Cannot train binary classifier."
            )
        ...
        if total_rows < min_rows:
            raise RuntimeError(
                f"CRITICAL: Insufficient data ({total_rows} rows). Need at least {min_rows:,} rows for {'debug' if min_rows == 100 else 'full'} phase."
            )
            ...
            if feature_set:
                ...
                if max_missing_rate > 0.2:
                    self.warnings.append(
                        f"High missing rate ({max_missing_rate:.2%}) in {feature_set[0][:10]}... features"
                    )
            ...
            removed_df = pd.DataFrame(
                {
                    ...
                    "reason": ["constant"] * len(constant_features)
                    + ["near_constant"] * len(near_constant_features),
                }
            )
    ...
    def validate_split_integrity(
        ...
        families_series: pd.Series | None = None,
    ) -> dict[str, Any]:
        ...
        if time_split and "timestamp" in train_data.columns:
            ...
            if train_max_ts >= test_min_ts:
                self.warnings.append(
                    "Time split integrity issue: train max timestamp >= test min timestamp"
                )

            # Log counts by month
            train_monthly = (
                train_data["timestamp"].dt.to_period("M").value_counts().sort_index()
            )
            test_monthly = (
                test_data["timestamp"].dt.to_period("M").value_counts().sort_index()
            )
            ...
            stratified_path = (
                self.settings.artifacts_dir / "debug_full_split_stratified.json"
            )
        ...
        if leakage_details:
            ...
            pd.DataFrame(columns=["column", "overlap_count", "overlap_ids"]).to_csv(
                leakage_path, index=False
            )
    ...
    def generate_debug_report(
        self, test_metrics: Any, model: Any, training_summary: dict[str, Any]
    ) -> str:
        ...
        if auroc <= 0.55:
            ...
            if (
                self.debug_data["data_summary"]["prevalence"] < 0.01
                or self.debug_data["data_summary"]["prevalence"] > 0.99
            ):
            ...
            if training_summary["best_iteration"] > 2500:
                probable_causes.append(
                    "Model overfitting - try increasing min_data_in_leaf or reducing num_leaves"
                )
        ...
        if auroc <= 0.55:
            ...
            print("📋 See artifacts/debug_full_report.json for details.")
            if probable_causes:
    ...
    def _generate_markdown_report(self, report: dict[str, Any]) -> str:
        ...
        for i, feature in enumerate(
            report["training_summary"]["top_20_features"][:20], 1
        ):
