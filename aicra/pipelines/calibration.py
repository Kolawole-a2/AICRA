...
class CalibrationPipeline:
    ...
    def run(
        ...
        if method == "auto":
            method = self._select_best_calibration_method(
                y_prob_train, train_data.labels.values
            )
            if not skip_mlflow:
        ...
        self._plot_reliability_diagram(
            val_data.labels.values, y_prob_val, y_prob_calibrated, method
        )
    ...
    def _select_best_calibration_method(
        ...
        try:
            ...
            platt_calibrator = CalibratedClassifierCV(
                dummy_clf, method="sigmoid", cv=cv_folds
            )
            ...
            isotonic_calibrator = CalibratedClassifierCV(
                dummy_clf, method="isotonic", cv=cv_folds
            )
    ...
    def _plot_reliability_diagram(
        ...
        ax2.plot(
            prob_pred_cal, prob_true_cal, "o-", label=f"{method.title()} Calibrated"
        )
