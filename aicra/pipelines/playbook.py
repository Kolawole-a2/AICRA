...
def generate_attack_playbook(
    ...
    if threshold is not None:
        risk_col = "risk_score" if "risk_score" in register_df.columns else "probability"
    ...
    for idx, row in high_risk.iterrows():
        ...
        playbook["high_risk_assets"].append({
            ...
            "prescriptive_controls": prescriptive_controls if isinstance(prescriptive_controls, list) else [],
        })
    ...
    if "expected_loss" in register_df.columns:
        playbook["summary"]["total_expected_loss"] = float(register_df["expected_loss"].sum())
    ...
    for technique_id in all_techniques:
        ...
        playbook["recommendations"].append({
            ...
            "priority": "High" if technique_id in [t for asset in playbook["high_risk_assets"] for t in asset.get("techniques", [])] else "Medium",
            "affected_assets": len([a for a in playbook["high_risk_assets"] + playbook["medium_risk_assets"] if technique_id in a.get("techniques", [])]),
