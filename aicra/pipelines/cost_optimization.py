...
class CostOptimizer:
    ...
    def compute_cost_at_threshold(
        ...
        expected_cost = (
            self.false_negative_cost * fn_rate + self.false_positive_cost * fp_rate
        )
    ...
    def save_threshold_analysis(
        self, analysis: dict[str, float], output_path: Path | None = None
    ) -> Path:
    ...
    def save_threshold_table(
        self, threshold_table: pd.DataFrame, output_path: Path | None = None
    ) -> Path:
