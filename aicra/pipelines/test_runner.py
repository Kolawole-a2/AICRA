...
class TestRunnerPipeline:
        ...
        try:
            ...
            if phase == "smoke":
                ...
                return self._run_full_test(
                    data_dir, seed, debug, time_split, sample_size
                )
            else:
        ...
        with mlflow.start_run(tags={"phase": "smoke"}):
            ...
            if success:
                ...
                mock_metrics = type(
                    "MockMetrics",
                    (),
                    {"auroc": 1.0, "pr_auc": 1.0, "ece": 0.167, "lift_at_5pct": 2.0},
                )()
    ...
    def _run_small_ember_test(
        self, data_dir: str, sample_size: int, seed: int
    ) -> tuple[bool, str]:
        ...
        with mlflow.start_run(tags={"phase": "small_ember"}):
            try:
                ...
                train_data, test_data = self._load_real_ember_data(
                    data_dir, sample_size, seed, "small_ember"
                )
                ...
                # Calibrate model
                calibrator_path = self._calibrate_model(
                    model_path, train_data, test_data, "small_ember"
                )

                # Generate policy
                policy_path = self._generate_policy(
                    model_path, calibrator_path, test_data, "small_ember"
                )
                ...
                self._update_test_results_history(
                    "small_ember", len(train_data.features), metrics
                )
    ...
    def _run_full_test(
        ...
        with mlflow.start_run(tags={"phase": "full", "debug": debug}):
            try:
                ...
                train_data, test_data = self._load_real_ember_data(
                    data_dir, sample_size, seed, "full"
                )
                ...
                # Evaluate model
                metrics = self._evaluate_model_debug(
                    model, test_data, "full", debug_pipeline
                )

                # Generate debug report if in debug mode
                if debug and debug_pipeline:
                    report_path = debug_pipeline.generate_debug_report(
                        metrics, model, training_summary
                    )
                ...
                if not debug and model_path:
                    calibrator_path = self._calibrate_model(
                        model_path, train_data, test_data, "full"
                    )
                ...
                if calibrator_path:
                    policy_path = self._generate_policy(
                        model_path, calibrator_path, test_data, "full"
                    )
                ...
                # Update test results history
                self._update_test_results_history(
                    "full", len(train_data.features), metrics
                )
    ...
    def _load_real_ember_data(
        self, data_dir: str, sample_size: int | None, seed: int, phase: str
        ...
        features_df, labels_series, families_series, metadata = (
            data_loader.load_ember_data(
                data_dir=data_dir, sample_size=sample_size, seed=seed, phase=phase
            )
        )
    ...
    def _train_lightgbm_model(self, train_data: Dataset, phase: str) -> str:
        ...
        for seed in model_seeds:
            ...
            if n_samples < 10000:
                ...
                num_leaves = min(
                    31, max(7, n_samples // 100)
                )  # Scale with dataset size
                n_estimators = min(
                    100, max(10, n_samples // 50)
                )  # Scale with dataset size
                min_data_in_leaf = max(
                    1, n_samples // 1000
                )  # Ensure minimum data per leaf
            else:
    ...
    def _evaluate_model(self, model_path: str, test_data: Dataset, phase: str) -> Any:
        ...
        if phase != "smoke":
            ...
            if not coverage_ok:
                print(
                    f"⚠️  Coverage threshold warning for {phase} phase - continuing with reduced coverage"
                )
    ...
    def _evaluate_model_debug(
        self,
        model: Any,
        test_data: Dataset,
        phase: str,
        debug_pipeline: Any | None = None,
    ) -> Any:
    ...
    def _generate_register(
        ...
        if calibrator_path.exists():
            ...
            print(
                f"Warning: Calibrator not found at {calibrator_path}, using raw probabilities"
            )
    ...
    def _log_phase_metrics(
        self, phase: str, dataset_name: str, model_type: str
    ) -> None:
    ...
    def _generate_smoke_comparison(self, small_ember_metrics: Any) -> None:
        ...
        if smoke_metrics_path.exists():
            with open(smoke_metrics_path, encoding="utf-8") as f:
            ...
            for metric in ["auroc", "pr_auc", "ece", "lift_at_5pct"]:
                ...
                print(
                    f"{metric.upper():<15} {smoke_val:<10.3f} {small_val:<12.3f} {change_str:<10}"
                )

    def _update_test_results_history(
        self, phase: str, dataset_size: int, metrics: Any
    ) -> None:
    ...
    def _generate_comparison_plots(self) -> None:
        ...
        if history_path.exists():
            ...
            if len(df) >= 2:  # Need at least 2 phases for comparison
                ...
                plt.tight_layout()
                plt.savefig(
                    self.artifacts_dir / "phase_comparison.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close()
