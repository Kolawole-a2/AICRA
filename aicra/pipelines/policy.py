...
class Policy:
    ...
    impact_ranges: dict[str, dict[str, float]] = (
        None  # {"endpoint": {"min": 1e5, "max": 1e7}, ...}
    )
    scenario_impacts: dict[str, float] = (
        None  # {"data_breach": 5e6, "ransomware_encryption": 1e7, ...}
    )
    customer_segment_impacts: dict[str, float] = (
        None  # {"retail": 1e5, "commercial": 1e7, ...}
    )
...
class PolicyPipeline:
    ...
    def _load_risk_bucket_controls(self) -> dict[str, Any]:
        ...
        if not self.skip_mlflow:
            mlflow.log_param(
                "risk_bucket_controls_version", data.get("__version__", "unknown")
            )

        return data

    def compute_expected_loss(
        self, susceptibility_scores: np.ndarray, impact_values: np.ndarray | None = None
    ) -> np.ndarray:
        """Compute Expected Loss = S × Impact."""
        if impact_values is None:
            impact_values = np.full_like(
                susceptibility_scores, self.settings.impact_default
            )
    ...
    def create_policy(
        ...
        model_id: str = "",
        calibration_id: str = "",
        impact_table_path: Path | None = None,
        banking_config_path: Path | None = None,
    ) -> Policy:
        ...
        if banking_config_path and banking_config_path.exists():
            with open(banking_config_path, encoding="utf-8") as f:
    ...
    def save_policy(self, policy: Policy, output_path: Path | None = None) -> Path:
    ...
    def generate_ops_report(
        self, df: pd.DataFrame, policy: Policy, output_path: Path | None = None
    ) -> dict[str, Any]:
    ...
    def compute_lift_at_k_report(
        ...
        for k in k_values:
            ...
            # Compute lift
            precision_at_k = (
                top_k_df["label"].mean() if "label" in top_k_df.columns else 0
            )
    ...
    def enrich_register_with_controls(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["prescriptive_controls"] = df["susceptibility_bucket"].apply(
            get_controls_for_bucket
        )
