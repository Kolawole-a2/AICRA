...
class EvaluationPipeline:
    ...
    def run(
        ...
        if timestamp_column and timestamp_column in test_data.features.columns:
            ...
            families_test = (
                test_data.families[test_idx] if hasattr(test_data, "families") else None
            )
        else:
            if not skip_mlflow:
                mlflow.log_param("split_type", "random")
                mlflow.log_param(
                    "split_warning", "No timestamp column found, using random split"
                )
            y_prob_test = y_prob
            y_true_test = test_data.labels.values
            families_test = (
                test_data.families if hasattr(test_data, "families") else None
            )
        ...
        if family_column and families_test is not None:
            ...
            if not is_smoke_test:  # Skip coverage tracking for smoke tests
                family_list = (
                    families_test.tolist()
                    if hasattr(families_test, "tolist")
                    else list(families_test)
                )
        ...
        if not skip_mlflow:
            with mlflow.start_run():
                ...
                for k in k_values:
                    mlflow.log_metric(
                        f"lift_at_{k}pct", getattr(metrics, f"lift_at_{k}pct", 0.0)
                    )
    ...
    def _plot_roc_curve(self, y_true: np.ndarray, y_prob: np.ndarray, auroc: float):
        ...
        plt.savefig(
            self.settings.artifacts_dir / "roc.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_pr_curve(self, y_true: np.ndarray, y_prob: np.ndarray, pr_auc: float):
        ...
        plt.savefig(
            self.settings.artifacts_dir / "pr.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_confusion_matrix(
        ...
        plt.savefig(
            self.settings.artifacts_dir / "confusion.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _evaluate_out_of_family_generalization(
        ...
        canonical_families_test = [
            mapping_pipeline.normalize_family(f) for f in families
        ]

        # Split into in-family and out-of-family
        in_family_mask = np.array(
            [f != held_out_family for f in canonical_families_test]
        )
    ...
    def _compute_lift_at_k(
        self, y_true: np.ndarray, y_prob: np.ndarray, k: int
    ) -> float:
    ...
    def _plot_lift_curve(
        self, y_true: np.ndarray, y_prob: np.ndarray, k_values: list[int]
    ) -> None:
        ...
        plt.savefig(
            self.settings.artifacts_dir / "lift_curve.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_reliability_diagram(self, y_true: np.ndarray, y_prob: np.ndarray) -> None:
        ...
        plt.savefig(
            self.settings.artifacts_dir / "reliability.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
