...
def load_deterministic_lookup(path: Path) -> pd.DataFrame:
    ...
    LOGGER.info(f"Unique attacks: {df['attack_id'].nunique()}, Unique defenses: {df['defense_id'].nunique()}")
...
def get_embedding_model(model_name: str = "sentence-transformers/all-MiniLM-L6-v2") -> SentenceTransformer:
    ...
    LOGGER.info(f"Model loaded: {model_name} (embedding dimension: {model.get_sentence_embedding_dimension()})")
    return model
...
def embed_texts(model: SentenceTransformer, texts: List[str]) -> np.ndarray:
    ...
    """
    LOGGER.info(f"Embedding {len(texts)} texts using {model.get_sentence_embedding_dimension()}-dimensional embeddings")
...
def compute_cosine_similarity_matrix(
    ...
    """
    LOGGER.info(f"Computing cosine similarity matrix: {attack_embeddings.shape[0]} attacks × {defense_embeddings.shape[0]} defenses")
    ...
    LOGGER.info(f"Similarity score range: [{similarity_matrix.min():.4f}, {similarity_matrix.max():.4f}]")
...
def generate_learned_mapping(
    ...
    LOGGER.info("CRITICAL: Generating pairs PURELY from embedding similarity - NO filtering by deterministic pairs")
    ...
    LOGGER.info(f"Generated learned mapping with {len(learned_mapping_df)} attack-defense pairs")
    ...
    LOGGER.info(f"Average similarity score: {learned_mapping_df['similarity_score'].mean():.4f}")
    ...
    if deterministic_pairs is not None:
        ...
        if learned_pairs == deterministic_pairs:
            ...
            LOGGER.error("This should NEVER happen - learned mapping is based on embeddings, not deterministic pairs.")
            LOGGER.error("Possible causes:")
            LOGGER.error("  1. Embedding model is producing identical similarity scores")
            LOGGER.error("  2. Code is filtering/intersecting learned pairs with deterministic")
            LOGGER.error("  3. Deterministic pairs happen to match top-k embedding similarities")
            LOGGER.error("=" * 80)
            raise RuntimeError(
...
def build_embedding_learned_mapping(
    ...
    similarity_matrix = compute_cosine_similarity_matrix(attack_embeddings, defense_embeddings)
    ...
    deterministic_pairs_for_check = set(
        zip(df_deterministic["attack_id"], df_deterministic["defense_id"])
    )
    LOGGER.info(f"Extracted {len(deterministic_pairs_for_check)} deterministic pairs for verification ONLY (NOT for filtering)")
    LOGGER.info("CRITICAL: Learned mapping will be generated PURELY from embedding similarity, NOT filtered by deterministic pairs")
    ...
    LOGGER.info("=" * 80)
    LOGGER.info("VERIFICATION: Comparing learned vs deterministic using DataFrame merge")
    LOGGER.info("=" * 80)
    ...
    intersection = det_pairs.merge(learned_pairs, on=["attack_id", "defense_id"], how="inner")
    
    # Compute "only in deterministic" using left merge with indicator
    only_in_det = det_pairs.merge(learned_pairs, on=["attack_id", "defense_id"], how="left", indicator=True)
    only_in_det = only_in_det[only_in_det["_merge"] == "left_only"].drop(columns=["_merge"])
    
    # Compute "only in learned" using left merge with indicator
    only_in_learned = learned_pairs.merge(det_pairs, on=["attack_id", "defense_id"], how="left", indicator=True)
    only_in_learned = only_in_learned[only_in_learned["_merge"] == "left_only"].drop(columns=["_merge"])
