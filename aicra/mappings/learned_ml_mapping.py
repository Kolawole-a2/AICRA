...
def prepare_training_data(
    ...
    attack_to_defenses = (
        deterministic_mapping.groupby("attack_id")["defense_id"]
        .apply(list)
        .to_dict()
    )
...
def extract_text_features(
    ...
    if vectorizer is None:
        ...
        LOGGER.info(f"Fitted TF-IDF vectorizer with {len(vectorizer.vocabulary_)} features")
    else:
...
def train_classifier(
    ...
    LOGGER.info(f"Training on {X_train.shape[0]} examples with {X_train.shape[1]} features")
...
def generate_learned_mapping(
    ...
    defense_id_to_name = dict(zip(defense_catalog["defense_id"], defense_catalog["defense_name"]))
    
    for i, attack_id in enumerate(all_attacks["attack_id"]):
        attack_name = all_attacks[all_attacks["attack_id"] == attack_id]["attack_name"].iloc[0]
        defense_ids = predicted_defenses[i]
        confidences = confidence_scores[i]
        
        # Get defense indices for predicted defenses
        defense_indices = [mlb.classes_.tolist().index(did) for did in defense_ids if did in mlb.classes_]
...
def load_deterministic_lookup(path: Path) -> pd.DataFrame:
    ...
    num_unique_defenses = df["defense_id"].nunique() if "defense_id" in df.columns else 0
    
    LOGGER.info(f"Loaded {num_rows} rows")
    LOGGER.info(f"Unique attacks: {num_unique_attacks}, Unique defenses: {num_unique_defenses}")
...
def build_attack_to_defenses(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped["defense_ids"] = grouped["defense_ids"].apply(lambda x: list(set(x)) if isinstance(x, list) else [])
...
def prepare_text_features(df_attacks: pd.DataFrame) -> Tuple[np.ndarray, TfidfVectorizer]:
    """
    ...
    LOGGER.info(f"Prepared TF-IDF features: {X.shape[0]} samples, {X.shape[1]} features")
    
    return X.toarray(), vectorizer


def prepare_labels(df_attacks: pd.DataFrame) -> Tuple[np.ndarray, MultiLabelBinarizer, List[str]]:
    """
...
def build_learned_mapping_table(
    ...
    """
    LOGGER.info(f"Building learned mapping table for {len(df_attacks)} attacks (top_k={top_k})")
    ...
    LOGGER.info(f"Built learned mapping: {num_attacks} unique attack_ids, {num_pairs} attack-defense pairs")
...
def save_learned_mapping(df_learned: pd.DataFrame, out_dir: Path) -> None:
    ...
    LOGGER.info(f"Learned mapping stats:")
