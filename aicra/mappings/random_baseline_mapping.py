...
def build_random_baseline_mapping(
    ...
    LOGGER.info(f"  Average controls per attack: {len(random_mapping) / len(unique_attacks):.2f}")
...
def generate_and_save_random_baseline(
    ...
    if not deterministic_path.exists():
        raise FileNotFoundError(f"Deterministic mapping not found: {deterministic_path}")
